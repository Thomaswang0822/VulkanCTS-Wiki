...
- **Flush and invalidate direction.** For a mapped non-coherent range, [`vkFlushMappedMemoryRanges`](../../../vulkan-docs/src/chapters/memory.adoc#L5180-L5240) handles host writes intended for later device access. It makes those writes available so a later memory dependency can make them visible to the device. [`vkInvalidateMappedMemoryRanges`](../../../vulkan-docs/src/chapters/memory.adoc#L5308-L5364) handles device writes intended for a later host read. After synchronization makes the writes available to the host domain, invalidation lets the host see them instead of stale cached bytes. Neither operation waits for device work or defines the required memory dependency.
