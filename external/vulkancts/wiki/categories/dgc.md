...
The category contains the NV device-generated-command branch and the EXT branch. The root dispatcher registers these branches in [`vktDGCTests.cpp`](../../modules/vulkan/device_generated_commands/vktDGCTests.cpp#L69-L130); the implementation details are split across the Level-3 pages below.
...
| `dgc.ext.compute.push_index_heap` | [ComputePushIndexHeapExt](../testfiles/dgc/ComputePushIndexHeapExt.md) | EXT compute cases where a DGC-pushed index selects a descriptor-heap slot and the routed write is verified. |
...
The root dispatcher now places the existing ray-tracing tests under `dgc.ext.ray_tracing.basic` and adds `dgc.ext.ray_tracing.conditional_rendering` and `dgc.ext.stat_query` ([registration](../../modules/vulkan/device_generated_commands/vktDGCTests.cpp#L114-L125)). The existing [RayTracingExt](../testfiles/dgc/RayTracingExt.md) page covers `basic`; the new families have the following source-backed scope.
