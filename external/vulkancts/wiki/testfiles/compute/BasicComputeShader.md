...
- `computePipelineConstructionType == SHADER_OBJECT_SPIRV|SHADER_OBJECT_BINARY` excludes the Amber regression cases (skipped when `isComputePipelineConstructionTypeShaderObject(...)` is true). The non-VulkanSC `replicated_composites_*` subtree is not inside that shader-object condition and executes under all three construction roots [vktComputeBasicComputeShaderTests.cpp](../../../modules/vulkan/compute/vktComputeBasicComputeShaderTests.cpp#L5922-L5992).
- Several Amber cases add `shaderFloat64`, `shaderFloat16`, `storageBuffer16BitAccess`, `uniformAndStorageBuffer16BitAccess`, `shaderInt16`, and `shaderDenormPreserveFloat16` requirements [vktComputeBasicComputeShaderTests.cpp](../../../modules/vulkan/compute/vktComputeBasicComputeShaderTests.cpp#L5960-L5989).
