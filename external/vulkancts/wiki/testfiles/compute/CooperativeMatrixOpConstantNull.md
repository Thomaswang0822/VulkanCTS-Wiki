...
This walkthrough uses one exact viable static configuration from `computePossibleConfigurationsStatic()`:

| Parameter choice | Value |
|------------------|-------|
| Target slot | `null_b`; the push constant selects `MAT_B`, so `loadMatB_def` stores `%matB_null` instead of loading SSBO binding 1. |
| Component types | `A = VK_COMPONENT_TYPE_UINT32_KHR`, `B = VK_COMPONENT_TYPE_UINT32_KHR`, `C = VK_COMPONENT_TYPE_UINT32_KHR`, `Result = VK_COMPONENT_TYPE_UINT32_KHR`. A/B are integer types (therefore not the rejected FP32/FP64 operands), and C is an integer accumulator as required by the static rules. |
| Sizes | `M = 8`, `K = 8`, `N = 8`; all are members of `PossibleSizes` and multiples of four. The emitted module carries `OpSpecConstant` values for these dimensions; the host specializes them for this configuration. |
| Scope | `VK_SCOPE_SUBGROUP_KHR`, encoded as `%uint_3` in each `OpTypeCooperativeMatrixKHR`. |
| Pipeline construction | `pipeline`; the same module is also registered for the other supported construction roots. |

The authoritative source is the CTS-authored SPIR-V template in `genShaderCode(conf)`, called by `initPrograms()` and registered through `SpirVAsmBuildOptions` for SPIR-V 1.3 [template](../../../modules/vulkan/compute/vktComputeCooperativeMatrixOpConstantNullTests.cpp#L779-L1139), [registration](../../../modules/vulkan/compute/vktComputeCooperativeMatrixOpConstantNullTests.cpp#L1352-L1371). This is not reconstructed GLSL and is not a module generated by glslang.
