...
The main matrix has generated graphics and compute observer programs. The representative leaf below is:
...
|------------------|---------|
| `draw` | Uses a fullscreen vertex/fragment pipeline to observe the copied sampled image. |
| `host_transition_host_copy` | Uses `vkTransitionImageLayoutEXT` and a host copy rather than a pipeline-barrier transition/queue copy route. |
| `memory_to_image` | Uploads generated host bytes through `vkCopyMemoryToImageEXT`. |
| `r8g8b8a8_unorm` pair | Samples RGBA8 input and writes RGBA8 output. |

### Purpose

The vertex shader derives fullscreen texture coordinates from `gl_VertexIndex`. The fragment shader samples the copied input image at those coordinates and writes the sampled RGBA value to the output attachment. It makes a host-side copy observable through ordinary graphics sampling.

### Structural Design

| Program | Resources and action | Test role |
|---------|----------------------|-----------|
| Vertex | No bound resources; derives `texCoord` and clip position from `gl_VertexIndex`. | Covers the output attachment with a fullscreen primitive. |
| Fragment | Binding 0: `sampler2D combinedSampler`; location 0 output: `vec4 out_color`. | Samples the copied image and writes its observed value. |
| Compute alternative | Binding 0: sampler; binding 1: write-only `image2D` or `uimage2D`; local size `(1,1,1)`. | One invocation samples one copied input pixel and stores it to the output image. |

### Generated Shader Code

```glsl
// Vertex shader generated by HostImageCopyTestCase::initPrograms().
#version 450
...
```glsl
// Fragment shader generated for a non-depth/stencil sampled format.
#version 450
...
For sampled depth/stencil formats, the generated fragment output is `vec4(texture(combinedSampler, texCoord).r, 0, 0, 0)`. The dispatch path chooses `uimage2D` and scales sampled values by 255 for the RGBA8_UINT output pair; otherwise it uses `image2D` and writes the sampled `vec4` directly ([program generator](../../../modules/vulkan/image/vktImageHostImageCopyTests.cpp#L1347-L1405)).
