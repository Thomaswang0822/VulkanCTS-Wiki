...
The same `block_matching` test family is also registered below `graphics.fast_lib`, `graphics.shader_objects`, and `compute`. The category dispatcher selects the graphics construction type and compute path in [`createChildren()`](../../../modules/vulkan/image_processing/vktImageProcessingTests.cpp#L45-L87); the common factory creates the `sad` and `ssd` operation families in [`createImageProcessingBlockMatchingCommonTests()`](../../../modules/vulkan/image_processing/vktImageProcessingBlockMatchingTests.cpp#L1881-L1898).
...
| Final comparison | [`verifyResult()`](../../../modules/vulkan/image_processing/vktImageProcessingBase.cpp#L231-L266) | Compares the output image exactly and the metric within tolerance. |
