...
This page covers the `acceleration_structures` test family registered by [acceleration-structure case registration](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4744-L4768).
...
|-----------|--------|-------------------|----------------------|----------|
| Residency | most families | `traditional_structures`, `sparse_binding_structures` | Where the acceleration-structure buffer memory lives; cross-gates host-build cases. | [Residency](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3528-L3536) |
| Shader source / pipeline | most families | vert, tesc, tese, geom, frag, comp, rgen, isect, ahit, chit, miss, call | Selects the pipeline that runs the ray query; stage-specific feature gates apply. | [Shader source / pipeline](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3539-L3561) |
| Build type | most families | `cpu_built` (HOST_KHR), `gpu_built` (DEVICE_KHR) | Where the BLAS/TLAS build runs. | [Build type](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3562-L3570) |
| Bottom geometry | most families | `triangles`, `aabbs` (plus `triangles_aop` and `aabbs_aop` for `flags`) | The BLAS geometry and whether it uses arrays-of-pointers. | [Bottom geometry](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3571-L3582) |
| Top instance pattern | most families | `identical_instances`, `different_instances` (plus `*_aop`) | Single-BLAS-many-instances vs one-instance-per-geometry placement. | [Top instance pattern](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3583-L3594) |
| Padding | `flags`, `format` | `nopadding`, `padded` | Whether vertex stride matches the BLAS input layout. | [Padding](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3622-L3629) |
| Build flags (4 axes) | `flags` | optimization (`0/fasttrace/fastbuild`), update (`0/update`), compaction (`0/compaction`), lowmemory (`0/lowmemory`) | Combines the four flags into one name token; encoded into the leaf name. | [Build flags (4 axes)](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3595-L3621) |
| Generic creation | `flags` | `""`, `_bottomgeneric`, `_topgeneric`, `_bothgeneric` | Whether the BLAS and/or TLAS are created as generic (no typed create info). | [Generic creation](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3631-L3641) |
| Vertex format | `format` | 15 listed `VkFormat`s (e.g. `R32G32B32_SFLOAT`, `R16G16B16_SFLOAT`, `R64G64B64A64_SFLOAT`) | The BLAS vertex buffer format; format-specific texture layout. | [Vertex format](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3776-L3887) |
| Index format | `format`, `instance_triangle_culling`, `empty` | `index_none`, `index_uint16`, `index_uint32` | The BLAS index buffer type where supported. | [Index format](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3879-L3887) |
| Operation type | `operations`, `instance_update`, `host_threading` | `copy`, `compaction`, `serialization`, `update`, `update_in_place` | The AS post-build operation. | [Operation type](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4048-L4056), [L4476-L4484](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4476-L4484) |
| Device address command path | operation leaves with `_dac` suffix | Uses device-address command variants for the eligible compute/fragment operation cases; host builds and sparse-binding combinations are pruned. | [Device address command path](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4120-L4168) |
| Operation target | `operations` | `top_acceleration_structure`, `bottom_acceleration_structure` | Which AS the operation acts on. | [Operation target](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4067-L4074) |
| Worker threads | `host_threading` | `1`, `2`, `3`, `4`, `8`, `max` | Number of CPU threads joining the deferred copy or serialization operation. | [Worker threads](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4176-L4190) |
| Function-call representation | `function_argument` | fixed nested pointer-to-value path | The outer SPIR-V wrapper receives a UniformConstant pointer, loads the AS, and passes the bare value to the inner wrapper; this is one mechanism inside every leaf, not a registered choice. | [Function-call representation](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L2199-L2489) |
| Cull flags | `instance_triangle_culling` | `noflags`, `ccw`, `nocull`, `ccw_nocull` | Per-instance face-culling flags combined with the back-face cull ray flag. | [Cull flags](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4339-L4357) |
| Empty case | `empty` | `inactive_triangles`, `inactive_instances`, `no_geometries_bottom`, `no_primitives_top`, `no_primitives_bottom` | The kind of empty AS the build produces. | [Empty case](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4664-L4674) |
| Dynamic-index form | `dynamic_indexing` | `nonuniformEXT(tlasIndex)` into `tlasArray[]`, or `*tlasPointers.ptr[nonuniformEXT(...)]` | The way the shader picks a TLAS for the ray query. | [Dynamic-index form](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3020-L3068) |
...
- The shader body is the verbatim compute wrapper from [`RayQueryASBasicTestInstance()`](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1667-L2198) with the triangle ray-query fragment from [`RayQueryASBasicTestInstance()`](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1677-L1695) spliced in. The build options are `vk::ShaderBuildOptions` with `SPIRV_VERSION_1_4` ([`RayQueryASBasicTestInstance()`](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1669)).
- In generated GLSL, the AABB variant replaces the triangle-candidate check with `gl_RayQueryCandidateIntersectionAABBEXT` ([AABB-candidate query fragment](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1697-L1712)); the hand-written `function_argument` and `dynamic_indexing` shaders are separate mechanisms.
- `instance_triangle_culling` substitutes `gl_RayFlagsCullBackFacingTrianglesEXT` for the `0` ray flag when `cullFlags != NONE` ([back-face culling ray flag selection](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1684)); this is the only shader-side parameter variation across the seven generated-GLSL families.
...
|---------------------|---------------------------------------|----------|
| `BottomTestType` | AABB replaces `gl_RayQueryCandidateIntersectionTriangleEXT` with `gl_RayQueryCandidateIntersectionAABBEXT`; the rest of the body stays the same. | [BottomTestType](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1697-L1712) |
| `InstanceCullFlags` (in `instance_triangle_culling`) | Ray flag value flips from `0` to `gl_RayFlagsCullBackFacingTrianglesEXT` when `cullFlags != NONE`. | [InstanceCullFlags (in instancetriangleculling)](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1683-L1685) |
| Empty AS (`empty` family, non-`NOT_EMPTY` cases) | Same body fragment, no candidate appears, both layers stay at `(0, 0)`. | [Empty AS (empty family, non-NOTEMPTY cases)](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L871-L901) |
| `function_argument`, `dynamic_indexing` | Switch from generated GLSL to hand-written SPIR-V with an explicit wrapper function or `nonuniformEXT` indexing. | [functionargument, dynamicindexing](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L2199-L2498), [functionargument, dynamicindexing](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3018-L3342) |
...
- All leaves require `VK_KHR_acceleration_structure` and `VK_KHR_ray_query` (via `commonASTestsCheckSupport` at [`RayQueryASBasicTestInstance()`](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1606-L1608)).
- A subset of `flags` cases additionally requires `VK_KHR_device_address_commands` ([`RayQueryASBasicTestInstance()`](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1610-L1611)).
- Tessellation-control, tessellation-evaluation, geometry, vertex, and ray-tracing stages require their stage-specific feature bits ([`RayQueryASBasicTestInstance()`](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1613-L1646)).
- Host-built (`cpu_built`) cases require `VkPhysicalDeviceAccelerationStructureFeaturesKHR::accelerationStructureHostCommands` ([`RayQueryASBasicTestInstance()`](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1648-L1653)).
- Each vertex format is checked through `checkAccelerationStructureVertexBufferFormat` ([`RayQueryASBasicTestInstance()`](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1656-L1657)).
- Sparse-binding residency requires `VkPhysicalDeviceFeatures::sparseBinding` ([`RayQueryASBasicTestInstance()`](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1660-L1664)).

### Design-based pruning

- The cross `cpu_built × sparse_binding_structures` is pruned in every family because Vulkan host commands cannot write into sparse-backed buffer memory. The pruning appears at [host-build sparse-residency pruning](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3695-L3699), [L4228-L4230](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4228-L4230), [L4407-L4409](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4407-L4409), [L4500-L4502](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4500-L4502), and [L4701-L4703](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4701-L4703).
- The `_device_address` suffix is registered only for `flags` cases that match a small predicate on the matrix index ([device-address case selection predicate](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3746-L3756)).
- The `host_threading` matrix is restricted to `OP_COPY`/`OP_SERIALIZE` and to host-built ASes ([worker-thread operation and build-type restrictions](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4098-L4111)).
- The `function_argument` family uses only the compute pipeline and triangle BLAS ([`CallValue()`](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L2203-L2204)).
- The `instance_triangle_culling` family uses only triangle BLAS; the cull flag dimensions do not apply to AABBs ([instance-culling parameter loops](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4418)).
...
| `RayQueryASBasicTestCase::checkSupport` | [RayQueryASBasicTestCase::checkSupport](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1606-L1665) | Feature gates and per-leaf support checks. |
| `RayQueryASBasicTestCase::initPrograms` (per-stage GLSL wrapper) | [Per-stage GLSL wrappers for acceleration-structure ray queries](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1667-L2198) | The per-stage shader wrapper that splices the per-`BottomTestType` ray-query body. |
| `RayQueryASFuncArgTestCase::initPrograms` (SPIR-V wrapper function) | [RayQueryASFuncArgTestCase::initPrograms (SPIR-V wrapper function)](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L2199-L2498) | The hand-written SPIR-V for the function-argument family. |
| `RayQueryASBasicTestInstance::iterateNoWorkers` / `iterateWithWorkers` | [RayQueryASBasicTestInstance::iterateNoWorkers / iterateWithWorkers](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L2929-L2971) | Single-thread vs. multi-thread orchestration. |
| `RayQueryASDynamicIndexingTestCase::initPrograms` | [Dynamic-indexed acceleration-structure SPIR-V generation](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3018-L3342) | The hand-written SPIR-V for the dynamic-indexing family. |
| `GraphicsConfiguration::verifyImage`, `ComputeConfiguration::verifyImage`, `RayTracingConfiguration::verifyImage` | [GraphicsConfiguration::verifyImage, ComputeConfiguration::verifyImage, RayTracingConfiguration::verifyImage](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L682-L690), [L871-L901](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L871-L901), [L1149-L1181](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L1149-L1181) | Reference image builders and `intThresholdCompare`. |
| `addBasicBuildingTests` | [addBasicBuildingTests](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3522-L3773) | Registers `flags`. |
| `addVertexIndexFormatsTests` | [addVertexIndexFormatsTests](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3776-L3972) | Registers `format`. |
| `addOperationTestsImpl` | [addOperationTestsImpl](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L3975-L4168) | Registers `operations` and is reused by `host_threading`. |
| `addHostThreadingOperationTests` | [Host-thread-count groups for acceleration-structure operations](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4176-L4190) | Thread-count grouping `1`/`2`/`3`/`4`/`8`/`max`. |
| `addFuncArgTests` | [addFuncArgTests](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4193-L4263) | Registers `function_argument`. |
| `addInstanceTriangleCullingTests` | [addInstanceTriangleCullingTests](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4266-L4448) | Registers `instance_triangle_culling`. |
| `addInstanceUpdateTests` | [addInstanceUpdateTests](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4450-L4538) | Registers `instance_update`. |
| `addDynamicIndexingTests` | [addDynamicIndexingTests](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4540-L4570) | Registers `dynamic_indexing`. |
| `addEmptyAccelerationStructureTests` | [addEmptyAccelerationStructureTests](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4572-L4742) | Registers `empty` with five empty-AS cases. |
| `createAccelerationStructuresTests` | [Acceleration-structure ray-query family registration](../../../modules/vulkan/ray_query/vktRayQueryAccelerationStructuresTests.cpp#L4744-L4768) | Top-level registration. |
