...
- Generated maximal compute cases compare the raw `tcu::UVec4` output buffer against the CPU reference with `deMemCmp`; on a mismatch they scan vector entries and construct ballot objects only for the first five diagnostics ([comparison](../../../modules/vulkan/reconvergence/vktReconvergenceTests.cpp#L5334-L5374)). The compute path touches the watchdog before submission. Allocation and reallocation guards across compute and graphics paths catch `const tcu::TestError &` and return a quality warning for oversized/unavailable output allocations, rather than treating those allocation exceptions as conformance failures ([compute allocation](../../../modules/vulkan/reconvergence/vktReconvergenceTests.cpp#L5107-L5127), [reallocation](../../../modules/vulkan/reconvergence/vktReconvergenceTests.cpp#L5230-L5250)). These host changes do not alter the registered hierarchy or the fixed Amber shader below.
