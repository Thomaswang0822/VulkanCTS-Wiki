...
- The `1 << 1` literals are generated by substituting `selectedSample = 1` into the SELECT template at [SELECT branch](../../../modules/vulkan/renderpass/vktRenderPassSampleReadTests.cpp#L1079-L1092); for sample `N` the constant is `1 << N`.
...
|-------------|------|----------------|
| `createRenderPassSampleReadTests` | [create the multisample input-read group](../../../modules/vulkan/renderpass/vktRenderPassSampleReadTests.cpp#L1181-L1184) | Registers the `"sampleread"` group. |
| `initTests` | [enumerate sample counts, read modes, and selected samples](../../../modules/vulkan/renderpass/vktRenderPassSampleReadTests.cpp#L1143-L1177) | Generates the sample-count × mode × selected-sample matrix and applies non-monolithic pruning. |
| `Programs::init` | [Programs::init](../../../modules/vulkan/renderpass/vktRenderPassSampleReadTests.cpp#L1026-L1098) | Generates the vertex, subpass-1 color, and subpass-2 validation shaders. |
| `createRenderPass` (template) | [build two subpasses with an input-attachment dependency](../../../modules/vulkan/renderpass/vktRenderPassSampleReadTests.cpp#L204-L361) | Builds the two-subpass render pass and the input-attachment dependency. |
| `iterateInternal` / `iterateInternalDynamicRendering` | [iterateInternal / iterateInternalDynamicRendering](../../../modules/vulkan/renderpass/vktRenderPassSampleReadTests.cpp#L662-L811) | Host-side execution for legacy/renderpass2 and dynamic rendering. |
| `verifyResult` | [require every sample-read result to equal one](../../../modules/vulkan/renderpass/vktRenderPassSampleReadTests.cpp#L1004-L1024) | Final all-`1.0` comparison with zero threshold. |
| `checkSupport` | [checkSupport](../../../modules/vulkan/renderpass/vktRenderPassSampleReadTests.cpp#L1110-L1141) | Feature, extension, and format/sample-count support gates. |
