...
- This page covers the implementation and registrations created by [`createBasicTests()`](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L3609-L4230).
...
The tree deliberately stops after the family component. The `vk-default` mustpass contains 106,008 leaves owned by this page across these eight roots; `basic` is only one family and is not the name of the whole page's registration space. The format, rate, attachment, shader, sample, and other generated dimensions are descendants of each listed family and are omitted from the compact tree. The pipeline-library and fast-linked-library roots have no `basic` family, but their listed families still belong to this page. `ds_baselayer`, `ds_baselevel`, and `ds_baselevel_baselayer` are registered only under the first root, because a depth/stencil base-subresource case requires renderpass2, a real fragment shading rate attachment, and one sample. `misc_tests` is the Basic implementation's fixed-case branch; the separate `misc` branch belongs to `Misc.md` and is intentionally not listed here. Vulkan SC registers the renderpass2 monolithic root only. The parent creates these rendering, command-buffer, and pipeline-construction permutations before calling `createBasicTests()` in [`createTests()`](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateTests.cpp#L534-L557).

## Parameter Dimensions and Observed Values

| Dimension | Registered values | Meaning in this test | Evidence |
|-----------|-------------------|----------------------|----------|
| Test family | `basic`, `apisamplemask`, `samplemaskin`, `conservativeunder`, `conservativeover`, `ds_baselevel`, `ds_baselayer`, `ds_baselevel_baselayer`, `fragdepth`, `fragstencil`, `multiviewport`, `colorlayered`, `srlayered`, `multiview`, `multiviewsrlayered`, `multiviewcorrelation`, `interlock`, `samplelocations`, `sampleshadingenable`, `sampleshadinginput`, `fragdepth_early_late`, `fragstencil_early_late`, `fragdepth_clear`, `fragstencil_clear`, `fragdepth_baselevel`, `fragstencil_baselevel`, `multipass`, `multipass_fragdepth`, `multipass_fragstencil`, `maintenance6`, `samplemaskout` | Selects the interaction under test. `ds_baselevel`, `ds_baselayer`, and `ds_baselevel_baselayer` are the combined depth/stencil base-subresource paths; the early-and-late and `maintenance6` families are excluded by `CTS_USES_VULKANSC`. | [`groupCases[]`](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L3713-L3768) |
| Depth/stencil base subresource | base mip level `0` or `1`, base array layer `0` or `1` | Fixed by the family rather than by a registered path component. `fragdepth_baselevel`, `fragstencil_baselevel`, `ds_baselevel`, and `ds_baselevel_baselayer` use mip level `1`; `ds_baselayer` and `ds_baselevel_baselayer` use array layer `1`. The depth/stencil image is enlarged and given extra layers so that the attachment view starts inside it. | [`CaseDef` fields](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L123-L124), [family assignment](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L3940-L3958) |
...
| Combiner 0 and combiner 1 | `keep`, `replace`, `min`, `max`, `mul` | Combines pipeline with primitive rate, then combines that result with attachment rate. `KEEP` and `REPLACE` select an input; `MIN` and `MAX` operate per dimension; `MUL` multiplies dimensions, subject to `fragmentShadingRateStrictMultiplyCombiner`. | [`combCases[]`](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L3693-L3699), [`Combine()`](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L858-L887) |
| Framebuffer extent | `1x1`, `4x4`, `33x35`, `151x431`, `256x256` | Exercises exact, odd, large, and square render areas. Odd extents also exercise fragment-region clamping at the framebuffer edge. | [`extentCases[]`](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L3701-L3703), [edge clamp](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L2690-L2701) |
...
| Attachment image modes | default, optimal layout, imageless framebuffer, 2D array, linear tiling | Expands the attachment path in `iterate()`. The 2D array mode matters for `srlayered`; the other modes exercise creation, layout, and framebuffer variants. | [`AttachmentModes`](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L1165-L1174), [attachment setup](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L1669-L1724) |
...
The standalone `misc_tests.sample_mask_test` case uses a `2x2` fragment size and mask `0x9`, which makes coverage loss inside a multi-pixel fragment visible. That case is registered by the same implementation block at [`misc_tests`](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L3953-L4001).
...
`fragdepth` writes `gl_FragDepth = float(instanceIndex) / 81.0`. The host compares each depth sample with `primID / NUM_TRIANGLES`. `fragstencil` writes `gl_FragStencilRefARB = instanceIndex`, and the host compares the stencil sample with `primID`. `ds_baselevel` exercises the combined depth/stencil attachment at mip level 1 and validates the corresponding base-level image behavior.

The early-and-late variants add `layout(early_and_late_fragment_tests_amd) in` and the AMD extension. The clear variants use a depth/stencil attachment clear operation and check the clear-path results. The base-level variants use depth or stencil attachment mip level `1`, and `ds_baselevel_baselayer` combines that mip level with a non-zero base array layer. Multipass variants use a second subpass and check fixed values written by the simple second-subpass fragment shader. `maintenance6` changes the combiner simulation according to `fragmentShadingRateClampCombinerInputs` and requires a fragment shading rate attachment.

`ds_baselayer` and `ds_baselevel_baselayer` attach a combined depth/stencil attachment whose framebuffer view does not start at the first array layer. The image is created with `dsBaseLayer + numColorLayers` array layers and `dsBaseMipLevel + 1` mip levels, and its extent is scaled by `1 << dsBaseMipLevel` so that the selected mip level matches the framebuffer. The combined, depth-only, and stencil-only views all use the same subresource range with `baseArrayLayer = dsBaseLayer`. These families are not layered, so `numColorLayers` is `1` and the image has exactly two array layers while the framebuffer renders one layer. The mip level below `dsBaseMipLevel` and the array layers below `dsBaseLayer` are never bound to the framebuffer, never cleared, and never read back [depth/stencil image and views](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L1373-L1422).

These two families do not write depth or stencil from the fragment shader, so `Force1x1()` does not apply to them. The pipeline always passes instead: `VK_COMPARE_OP_ALWAYS` with depth write enabled, and stencil `REPLACE` with reference `2` [depth/stencil state](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L2345-L2378). The vertex shader emits position z `0.4` whenever the case uses a depth/stencil attachment and the fragment shader does not write `gl_FragDepth` [vertex depth](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L471-L471). The host then requires every covered sample to read back depth `0.4` and stencil `2`, in addition to the expected shading rate [depth and stencil checks](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L2752-L2789). What the base-layer cases add over `ds_baselevel` is therefore an alignment check across three uses of the same offset: the rendered attachment view, the pre-render clear and layout transition, and the depth and stencil read-back views.
...
| `useSampleMaskIn` and `useSampleMaskOut` | Replaces `col0.y` with `gl_SampleMaskIn[0]`, and output cases mask it with `0x55555555` after writing `gl_SampleMask[0]`. | [fragment generator](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L711-L716) |
| `fragDepth` and `fragStencil` | Adds `gl_FragDepth` or `gl_FragStencilRefARB` writes with values derived from `instanceIndex`. | [depth and stencil generation](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L718-L722) |
| `interlock` | Adds ordered pixel interlock execution mode and begin/end calls around fragment work. | [interlock generation](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L659-L666), [interlock close](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L724-L725) |
| `sampleShadingInput` | Declares the `zero` input with `sample` interpolation and multiplies the error seed by `gl_SampleID`. | [sample input generation](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L642-L682) |
...
- `iterate()` queries supported fragment shading rates and properties, chooses a depth/stencil format when needed, creates the atomic counter and host-visible output buffers, and fills a random vertex buffer for `NUM_TRIANGLES` triangles [instance setup](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L1106-L1260).
...
- The depth and stencil textures in that dispatch are the depth-only and stencil-only views of the attachment image, bound at descriptor bindings 6 and 7, so the read-back covers the same mip level and array layers as the attachment view [view bindings](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L1854-L1862), [dispatch](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L2532-L2534).
- A depth/stencil case also limits its own preparation work: the pre-render layout transition and `vkCmdClearDepthStencilImage` both start at `dsBaseMipLevel` and `dsBaseLayer` and extend over the remaining array layers, so the subresources below the attachment view are neither transitioned nor cleared. That host-side clear of depth `0.0` and stencil `0` is what lets these families render with `VK_ATTACHMENT_LOAD_OP_LOAD` instead of a clear operation [pre-render depth/stencil setup](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L3193-L3210), [attachment description](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L2025-L2042).
- The host decodes rate flags into `pixelsX` and `pixelsY`, computes the fragment rectangle, clamps it to odd framebuffer dimensions, and derives pipeline, primitive, and attachment rates from the observed primitive and coordinate [rate simulation and region](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L2684-L2724).
- The host accepts the observed rate only when it appears in the mask returned by `Simulate()`. It then checks depth, stencil, viewport, layer, multiview, sample-mask, coverage, and same-fragment consistency as enabled by `CaseDef` [result checks](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L2721-L2920).
- Every nonzero fragment error code fails the case. Code `1` means fragment-center alignment, code `2` means pre-rasterization output readback, code `3` means explicit `gl_FragCoord` derivatives, and code `4` means implicit texture derivatives. If more than 16 samples all observe `1x1` while a larger expected mask exists, the test returns `quality_warning` rather than `fail` [error handling and quality warning](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L2642-L2680), [quality warning](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L2994-L3003).
...
| `fragdepth` / `fragstencil` / `ds_baselevel` / `ds_baselayer` / `ds_baselevel_baselayer` and their early-and-late, clear, base-level, base-layer, or multipass variants | Incorrect shader depth or stencil export, fixed-function test ordering, attachment base mip or base array layer selection, subpass handling, or readback. |
...
**Possible failure symptoms:** The host finds a written sample outside the API mask, a missing sample inside `gl_SampleMaskIn`, a non-full pixel under conservative or centered custom locations, a depth value different from `primID / NUM_TRIANGLES`, or a stencil value different from `primID`. In the depth/stencil base-subresource families the shaders write neither value, so a failure reports depth `0.0` or stencil `0`, the values the pre-render clear leaves behind, instead of the required `0.4` and `2`.

**Possible implementation causes:** The failure can come from fragment coverage, sample-mask application, shader export, depth/stencil test ordering, image transitions, or host copyback. The relevant specification interaction depends on the selected family and needs an API trace or implementation inspection when the log cannot distinguish them. A base-subresource failure narrows that search: the framebuffer attachment view, the pre-render clear and transition, and the read-back views all carry the same `dsBaseMipLevel` and `dsBaseLayer`, so the reported values point at subresource-range handling rather than at the rate calculation.
...
- Shader depth or stencil writes, API sample mask, shader sample mask, conservative, interlock, and custom sample-location cases force `1x1` when the corresponding interaction property is absent. Sample shading cases always force `1x1` and still require sample-rate shading [force behavior](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L279-L308). The condition tests `fragDepth` and `fragStencil`, so the depth/stencil base-subresource families, whose shaders write neither built-in, keep the full rate matrix.
- Dynamic rendering excludes `noattachmentptr` from the wrong path and omits `multiviewcorrelation` and `maintenance6`; renderpass2 excludes `attachment_noimageview`. Early-and-late and `maintenance6` are excluded from Vulkan SC. Secondary command buffers retain dynamic cases and reduce extent, sample, shader, and group selections. Pipeline-library permutations keep selected groups and every other extent [matrix pruning](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L3727-L3783).

### Design-based pruning

- Sibling families keep only `KEEP` and `REPLACE` combiner inputs, while `basic` retains all five combiner operations. This prevents interaction-specific families from duplicating the full combiner matrix [family pruning](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L3834-L3845).
- Geometry shaders are omitted for sample-mask, conservative, depth/stencil, maintenance6, sample-mask-output, non-dynamic-state, and other cases where that shader path is not part of the selected behavior. Layered and multiple-viewport groups require `shaderrate`. Clear and multipass depth/stencil cases require a real attachment, renderpass2, and one sample, and a nonzero depth/stencil base mip level or base array layer is pruned unless the case also has a real attachment, renderpass2, and one sample [case pruning](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L3968-L4016).
- `misc_tests.sample_mask_test`, `helper_invocation`, and non-SC `garbage_color_attachment` are fixed cases outside the generated matrix. Their registration and fixed `CaseDef` values are kept in [`misc_tests`](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L3953-L4099).
...
| Matrix registration | [createBasicTests()](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L3609-L4230) | Creates the family, state, attachment, shader, combiner, extent, sample, and pre-rasterization hierarchy. |
| Support and pruning | [FSRTestCase::checkSupport()](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L316-L437), [Force1x1()](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L279-L308) | Separates unsupported cases and interaction-driven `1x1` fallback from executed failures. |
| Shader generation | [FSRTestCase::initPrograms()](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L449-L834) | Emits vertex, geometry, mesh, fragment, compute, and second-subpass shaders. |
| Rate simulation | [Simulate() and `CombineMasks()`](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L886-L962) | Computes possible final rates with clamping and the maintenance6 input rule. |
| Runtime and result scan | [FSRTestInstance::iterate()](../../../modules/vulkan/fragment_shading_rate/vktFragmentShadingRateBasic.cpp#L1109-L3017) | Creates resources, submits rendering and copy work, and checks all observed outputs. |
