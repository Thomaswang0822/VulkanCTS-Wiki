...
| Tiling | `VK_IMAGE_TILING_OPTIMAL` only | Fixed by `DE_ASSERT` in the test case constructor | [Tiling](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L896) |
| Allocation kind | `ALLOCATION_KIND_SUBALLOCATED` only | Fixed by `DE_ASSERT` | [Allocation kind](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L897) |
| Queue selection | `Universal` only | Fixed by `DE_ASSERT`; no transfer-only variants | [Queue selection](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L898) |
...
|-------------|------|----------------|
| `ReinterpretTestInstance` class | [ReinterpretTestInstance class](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L35-L57) | Owns source/destination images, view format, and overrides `iterate()`, `checkTestResult()`, `copyRegionToTextureLevel()`. |
| Image creation with `MUTABLE_FORMAT_BIT` and `BLOCK_TEXEL_VIEW_COMPATIBLE_BIT` | [Image creation with MUTABLEFORMATBIT and BLOCKTEXELVIEWCOMPATIBLEBIT](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L59-L140) | Shows when each flag is added based on whether the format differs from the view and whether the format is compressed. |
| `copyRegionToTextureLevel` memcpy reference | [copyRegionToTextureLevel memcpy reference](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L152-L216) | Host-side reference for uncompressed copy verification; replaces destination format with source format to mimic memcpy. |
| `fillCompressedImages` (`compFill` dispatch) | [fillCompressedImages (compFill dispatch)](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L218-L311) | Dispatches `compFill` over `getSizeInBlocks`, binds storage image descriptors, records pre/post barriers. |
| `checkTestResult(testImage, ...)` (`compVerify` dispatch) | [checkTestResult(testImage, ...) (compVerify dispatch)](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L316-L466) | Dispatches `compVerify` against a test image, reads back the green/red output, and compares against an all-green reference. |
| `iterate()` end-to-end | [iterate() end-to-end](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L468-L863) | Wires upload/fill, copy, sampling renderpass, and verification; gates per-branch on `srcCompressed`. |
| `ReinterpretTestCase` constructor with `DE_ASSERT` constraints | [ReinterpretTestCase constructor with DEASSERT constraints](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L886-L905) | Documents the constrained parameter space. |
| `checkSupport()` | [checkSupport()](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L912-L979) | Gates `VK_KHR_maintenance2`, queries image format properties for both image and view formats, validates 1D/2D dimension limits. |
| `initPrograms()` shader generation | [initPrograms() shader generation](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L981-L1110) | Generates `vert`, `frag`, and (compressed only) `compFill` plus `compVerify`. |
| `createReinterpretationTests()` registration | [createReinterpretationTests() registration](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L1119-L1198) | Defines the three format pairs and two image types, registers leaves under `1d` and `2d`. |
| `formatsAreCompatible()` size check | [formatsAreCompatible() size check](../../../modules/vulkan/api/vktApiCopiesAndBlittingReinterpretTests.cpp#L881-L884) | Admits the uncompressed pair; compressed cases bypass this check. |
...
| Dispatcher registration | [Dispatcher registration](../../../modules/vulkan/api/vktApiCopiesAndBlittingTests.cpp#L290) | Adds `createReinterpretationTests()` directly under `copy_and_blit`. |
| `getSizeCompatibleTcuTextureFormat()` helper | [getSizeCompatibleTcuTextureFormat() helper](../../../modules/vulkan/api/vktApiCopiesAndBlittingUtil.cpp#L170-L177) | Maps compressed formats to size-compatible uncompressed tcu formats for host-side texture levels. |
