...
A `buffer_to_buffer_with_offset` sibling is registered under `core` only by [addCopyBufferToBufferOffsetTests()](../../../modules/vulkan/api/vktApiCopyBufferToBufferTests.cpp#L621). It generates 64 test case leaves (`0_0` through `7_7`) covering all `srcOffset`/`dstOffset` combinations from 0 to `kMaxOffset = 8`.
...
| Region offsets | aligned (0, 1024, 2048) and unaligned (1, 3, 6, 9, 11, 12, 16) | Aligned offsets test the basic path. Unaligned offsets test that the driver handles non-4-byte boundaries. | [addCopyBufferToBufferTests()](../../../modules/vulkan/api/vktApiCopyBufferToBufferTests.cpp#L421-L619) |
