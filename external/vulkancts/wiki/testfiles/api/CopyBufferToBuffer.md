...
| DAC pruning | [DAC pruning](../../../modules/vulkan/api/vktApiCopyBufferToBufferTests.cpp#L474) | Skips `regions` and `unaligned_regions` under `DEVICE_ADDRESS_COMMANDS` due to VUID-VkCopyDeviceMemoryInfoKHR-srcRange-13015. |
| DAC pruning (large) | [DAC pruning (large)](../../../modules/vulkan/api/vktApiCopyBufferToBufferTests.cpp#L594) | Skips `unaligned_regions_large` under `DEVICE_ADDRESS_COMMANDS` for the same VUID. |
...
| Test params and constants | [Test params and constants](../../../modules/vulkan/api/vktApiCopiesAndBlittingUtil.hpp#L161-L166) | Defines `defaultSize`, `defaultQuarterSize`, and `defaultLargeSize`. |
