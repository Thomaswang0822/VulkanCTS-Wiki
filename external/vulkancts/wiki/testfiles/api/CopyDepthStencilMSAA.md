...
- Test category: `api`. Composite family: `copy_and_blit`. Test family covered by this page: `depth_stencil_msaa_copy`, registered under two dispatcher intermediate nodes (`core` and `copy_commands2`) by [`addCopiesAndBlittingTests()`](../../../modules/vulkan/api/vktApiCopiesAndBlittingTests.cpp#L138). Implementation entry: [`addCopyDepthStencilMSAATests()`](../../../modules/vulkan/api/vktApiCopyDepthStencilMSAATests.cpp#L1260-L1275).
...
The same `depth_stencil_msaa_copy` test family with the same `whole` / `partial` / `array_to_array` subtree is also registered under `api.copy_and_blit.copy_commands2.depth_stencil_msaa_copy`. The `core` path records `vkCmdCopyImage` with `extensionFlags = NONE`; the `copy_commands2` path records `vkCmdCopyImage2` with `extensionFlags = COPY_COMMANDS_2`. Both paths share the same source implementation and parameter matrix; only the recorded command differs. Mustpass evidence starts at [`api.txt#L174767`](../../../mustpass/main/vk-default/api.txt#L174767) for `core.depth_stencil_msaa_copy` and [`api.txt#L23616`](../../../mustpass/main/vk-default/api.txt#L23616) for `copy_commands2.depth_stencil_msaa_copy`.
...
| Allocation kind | `ALLOCATION_KIND_SUBALLOCATION` (`core`), `ALLOCATION_KIND_DEDICATED` (`dedicated_allocation` parent, registered separately) | Selects how source and destination image memory is bound. The dedicated path skips the `_bind_offset` variant. | [`vktApiCopiesAndBlittingTests.cpp#L232-L246`](../../../modules/vulkan/api/vktApiCopiesAndBlittingTests.cpp#L232-L246) |
...
- The dual `core` and `copy_commands2` registration shares one source implementation; a failure on one path but not the other at the same parameter combination points to a `vkCmdCopyImage` vs. `vkCmdCopyImage2` divergence rather than a generic copy bug.
