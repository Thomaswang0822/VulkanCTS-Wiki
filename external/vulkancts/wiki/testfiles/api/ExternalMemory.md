...
- **Portability subset events.** `transference_*` cases on copy-transference handle types call [`checkEvent()`](../../../modules/vulkan/api/vktApiExternalMemoryTests.cpp#L4533-L4538) and are skipped when `VK_KHR_portability_subset` advertises `events == VK_FALSE`.
