...
The existing semaphore, memory, and fence matrices are supplemented by `misc.zero_handle_memory`, registered by [createMiscTests()](../../../modules/vulkan/api/vktApiExternalMemoryTests.cpp#L5499-L5507).
...
`misc.zero_handle_memory` chains `VkExportMemoryAllocateInfo` with `handleTypes = 0` into a host-visible allocation. A buffer supplies memory requirements, but the test neither binds that buffer nor exports a native handle. It writes and checks deterministic host data in the allocation ([testMemoryZeroHandleType()](../../../modules/vulkan/api/vktApiExternalMemoryTests.cpp#L3705-L3738)). This checks ordinary allocation behavior with a zero-valued export structure, not an export/import round trip.
