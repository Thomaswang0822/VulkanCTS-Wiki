...
|-------------|------|----------------|
| `AllocationKind` enum | [AllocationKind enum](../../../modules/vulkan/api/vktApiBufferViewCreateTests.cpp#L46-L52) | Defines the suballocation/dedicated_alloc dispatch dimension. |
| `BufferViewCaseParameters` struct | [BufferViewCaseParameters struct](../../../modules/vulkan/api/vktApiBufferViewCreateTests.cpp#L56-L64) | Carries per-case format, offset, range, usage, feature flag, and allocation kind. |
| `BufferViewTestCase::checkSupport()` | [BufferViewTestCase::checkSupport()](../../../modules/vulkan/api/vktApiBufferViewCreateTests.cpp#L120-L133) | Requirement-based pruning: format feature check and `VK_KHR_dedicated_allocation` gate. |
| `BufferSuballocation::createTestBuffer()` | [BufferSuballocation::createTestBuffer()](../../../modules/vulkan/api/vktApiBufferViewCreateTests.cpp#L139-L201) | Suballocation memory binding path with internal pass/fail checks. |
| `BufferDedicatedAllocation::createTestBuffer()` | [BufferDedicatedAllocation::createTestBuffer()](../../../modules/vulkan/api/vktApiBufferViewCreateTests.cpp#L203-L317) | Dedicated-allocation memory binding path, including the `requiresDedicatedAllocation == VK_FALSE` and `memoryTypeBits != 0` spec invariants. |
| `BufferViewTestInstance::iterate()` | [BufferViewTestInstance::iterate()](../../../modules/vulkan/api/vktApiBufferViewCreateTests.cpp#L319-L386) | Main test flow: allocator dispatch, primary view creation, "complete" view creation, pass/fail decision. |
| `createBufferViewCreateTests()` | [createBufferViewCreateTests()](../../../modules/vulkan/api/vktApiBufferViewCreateTests.cpp#L390-L444) | Family registration: builds the `suballocation` and `dedicated_alloc` intermediate nodes, the `uniform` and `storage` intermediate nodes, and one test case leaf per core Vulkan format. |
| Parent registration | [Parent registration](../../../modules/vulkan/api/vktApiTests.cpp#L78-L84) | `createBufferViewTests()` mounts `create` and the sibling `access` family under `buffer_view`; `createApiTests()` at [Parent registration](../../../modules/vulkan/api/vktApiTests.cpp#L106) mounts `buffer_view` under `api`. |
