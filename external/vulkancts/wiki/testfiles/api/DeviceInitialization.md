...
- It registers 27 intermediate nodes under `api.device_init`, each exercising one instance-creation, device-creation, or queue-creation scenario through [`createDeviceInitializationTests()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2816-L2895).
- Most intermediate nodes carry a single `basic` test case leaf; `create_device_queue_priorities` has `basic` and `concurrent_submission` leaves, while `create_device_unsupported_features` expands into one `core` leaf plus generated per-feature leaves via [`addSeparateUnsupportedFeatureTests()`](../../../framework/vulkan/generated/vulkan/vkDeviceFeatureTest.inl#L8679-L8679).
...
- No shaders are involved. Most cases check host-side creation/query behavior; queue-priority contention also submits buffer fills and validates their contents.
...
├── create_device_queue_priorities
...
The family is built in [`createDeviceInitializationTests()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2816-L2896) and attached to the `api` test category at [`vktApiTests.cpp`](../../../modules/vulkan/api/vktApiTests.cpp#L100). Each helper-created intermediate node wraps a single `basic` test case leaf through [`addFunctionCaseInNewSubgroup()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2839-L2866). The `create_device_unsupported_features` intermediate node is built explicitly: a `core` leaf via [`addFunctionCase(subgroup.get(), "core", createDeviceWithUnsupportedFeaturesTest)`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2899-L2899) plus 220 per-feature leaves generated from `vkDeviceFeatureTest.inl`.

## Parameter Dimensions and Observed Values

| Dimension | Registered values | Meaning in this test | Evidence |
|-----------|-------------------|----------------------|----------|
| Intermediate node | 27 names listed in the tree above | Each intermediate node selects one scenario: instance creation, device creation, queue configuration, global priority, queue-priority behavior, unsupported-feature rejection, or allocation-callback behavior. | [`createDeviceInitializationTests()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2816-L2895) |
| Queue-priority leaves | `basic`, `concurrent_submission` | `basic` creates queues with priorities spanning the implementation's discrete-priority range and checks queue retrieval/idle; `concurrent_submission` submits independent buffer fills to all eligible queues and checks host-visible buffer contents. | [`createDeviceWithQueuePrioritiesTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L887-L949), [`createDeviceWithQueuePrioritiesContentionTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L951-L1073) |
...
| Test case leaves under `create_device_unsupported_features` | `core` plus 220 per-feature leaves such as `16_bit_storage_features`, `buffer_device_address_features`, `descriptor_indexing_features`, `host_query_reset_features`, `robustness2_features_khr`, `dynamic_rendering_features`, ... | `core` enumerates the legacy `VkPhysicalDeviceFeatures` bitfields; each per-feature leaf targets one `VkBool32` member of an extension feature struct and expects `VK_ERROR_FEATURE_NOT_PRESENT` when that feature was reported unsupported. | [`addSeparateUnsupportedFeatureTests()`](../../../framework/vulkan/generated/vulkan/vkDeviceFeatureTest.inl#L8679-L8679); mustpass `dEQP-VK.api.device_init.create_device_unsupported_features.*` |
| Global priority value | `VK_QUEUE_GLOBAL_PRIORITY_LOW_KHR`, `VK_QUEUE_GLOBAL_PRIORITY_MEDIUM_KHR`, `VK_QUEUE_GLOBAL_PRIORITY_HIGH_KHR`, `VK_QUEUE_GLOBAL_PRIORITY_REALTIME_KHR` | Walked per queue family in the queried-priority variants; `HIGH` and `REALTIME` may be denied, out-of-range priorities must fail. | [`createDeviceWithGlobalPriorityTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L1081-L1238), [`createDeviceWithQueriedGlobalPriorityTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L1276-L1391) |
...
- `create_instance_name_version` iterates over `appNames`, `engineNames`, `appVersions`, `engineVersions`, and patch-number variants of the API version, plus an `apiVersion == 0` case; every combination must create an instance successfully ([`createInstanceTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L64-L207)).
- `create_instance_invalid_api_version` feeds deliberately overflowed variant/major/minor bitfields. Vulkan 1.0 may reject with `VK_ERROR_INCOMPATIBLE_DRIVER`; Vulkan 1.1 and later must not return `VK_ERROR_INCOMPATIBLE_DRIVER` for nonstandard but in-range variant versions ([`createInstanceWithInvalidApiVersionTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L209-L318)).
- `create_instance_null_appinfo` passes `pApplicationInfo = NULL` and expects success ([`createInstanceWithNullApplicationInfoTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L320-L349)).
- `create_instance_unsupported_extensions` enables `VK_UNSUPPORTED_EXTENSION` and `THIS_IS_NOT_AN_EXTENSION`; the result must be `VK_ERROR_EXTENSION_NOT_PRESENT` and no instance handle ([`createInstanceWithUnsupportedExtensionsTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L351-L394)).
- `create_instance_extension_name_abuse` and `create_instance_layer_name_abuse` walk seven UTF-8 abuse strings (long, illegal bytes, overlong NUL, overlong sequences, "zalgo", Chinese glyphs, empty string) as fake extension or layer names; each must be rejected with `VK_ERROR_EXTENSION_NOT_PRESENT` or `VK_ERROR_LAYER_NOT_PRESENT` respectively ([`createInstanceWithExtensionNameAbuseTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L469-L516), [`createInstanceWithLayerNameAbuseTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L518-L574)).

### Device enumeration and allocation tracking (`enumerate_devices_alloc_leak`)

Single intermediate node, excluded from Vulkan SC. Installs an `AllocationCallbackRecorder` around the instance, calls `vkEnumeratePhysicalDevices` twice, and asserts that the number of recorded allocations matches the number of non-NULL frees ([`enumerateDevicesAllocLeakTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L577-L626)). A non-zero allocation balance fails the test; an out-of-memory error other than `VK_ERROR_OUT_OF_HOST_MEMORY` (such as `VK_ERROR_OUT_OF_DEVICE_MEMORY`) from `vkEnumeratePhysicalDevices` produces a quality warning rather than continuing to the balance check, while `VK_ERROR_OUT_OF_HOST_MEMORY` falls through to the balance check.

### Queue-priority behavior (`create_device_queue_priorities`)

The two leaves exercise ordinary `VkDeviceQueueCreateInfo::pQueuePriorities` values, not implementation scheduling guarantees. Both cap the queue count at `discreteQueuePriorities + 1` and span priorities from 0.0 to 1.0 ([`makeSpanningQueuePriorities()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L879-L885)).

- `basic` creates the selected queues for every queue family, retrieves each queue with `vkGetDeviceQueue`, and requires a non-null handle and successful `vkQueueWaitIdle` ([`createDeviceWithQueuePrioritiesTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L887-L949)). It checks that priority values are accepted; it does not assert execution ordering or latency based on priority.
- `concurrent_submission` selects queue families with graphics, compute, or transfer capability and at least two queues, creates a 4 MiB host-visible buffer per queue, records `vkCmdFillBuffer` with a queue-specific sentinel, submits all command buffers before waiting, then checks every buffer element after fence completion ([`createDeviceWithQueuePrioritiesContentionTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L951-L1073)). A family with fewer than two eligible queues is skipped; if no family qualifies, the test is not supported.
...
- `create_device` is the smoke test: one queue family, one queue, `vkQueueWaitIdle` returns `VK_SUCCESS` ([`createDeviceTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L629-L671)).
- `create_multiple_devices` creates 5 devices (2 on Vulkan SC) on separate custom instances and waits each queue idle; partial success is reported through a `ResultCollector` ([`createMultipleDevicesTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L673-L746)).
...
- `create_device_global_priority` uses the EXT extension name ([`createDeviceWithGlobalPriorityTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L1081-L1238) with `useKhrGlobalPriority=false`).
...
This intermediate node has 221 mustpass leaves: `core` plus 220 generated feature-structure cases. Queue priorities also has a non-single-`basic` leaf shape.

- The `core` leaf enumerates 54 `VkBool32` fields of `VkPhysicalDeviceFeatures` (excluding `robustBufferAccess`, which is always supported), enables each unsupported feature one at a time, and requires `VK_ERROR_FEATURE_NOT_PRESENT` from `vkCreateDevice` ([`createDeviceWithUnsupportedFeaturesTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L1688-L1804)).
- The 220 per-feature leaves are generated from `vkDeviceFeatureTest.inl`. Each leaf targets one `VkBool32` member of an extension feature struct, queries the supported features, enables the unsupported member, and expects `VK_ERROR_FEATURE_NOT_PRESENT`. The shared driver is [`checkFeatures()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L1399-L1630), which also enables any prerequisite features the Vulkan spec mandates (for example `multiview` when `multiviewGeometryShader` is requested, `variablePointersStorageBuffer` when `variablePointers` is requested, `robustBufferAccess` when `robustBufferAccess2` is requested, `shaderImageFloat32Atomics` when `sparseImageFloat32Atomics` is requested).
...
- `create_device_queue2` is the smoke test: one protected-capable queue, retrieved via `vkGetDeviceQueue2` ([`createDeviceQueue2Test()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L1808-L1863)). It performs the same Vulkan 1.1 and `protectedMemory` check internally instead of through the registered support hook.
- `create_device_queue2_two_queues` is a smoke test that finds the first protected-capable family with `queueCount >= 2` and creates one protected plus one unprotected queue from it ([`createDeviceQueue2WithTwoQueuesSmokeTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2021-L2068)).
...
- `create_device_queue2_all_families_protected` does the same but adds `VK_DEVICE_QUEUE_CREATE_PROTECTED_BIT` to families that advertise `VK_QUEUE_PROTECTED_BIT` ([`createDeviceQueue2WithAllFamiliesProtected()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2368-L2406)).
- `create_device_queue2_all_combinations` builds the full cartesian product of per-family N+M splits across all protected-capable families and runs each combination in both original and reversed order ([`createDeviceQueue2WithMultipleQueueCombinations()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2235-L2332)).

All eight share [`runQueueCreationTestCombination()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L1967-L2019), which creates the device, retrieves every requested queue through `vkGetDeviceQueue2` with the matching flags, and fails the test if any retrieved queue is `VK_NULL_HANDLE` or any `vkQueueWaitIdle` returns a non-`VK_SUCCESS` result.

### Intentional allocation failure (`create_instance_device_intentional_alloc_fail`)

Single intermediate node, excluded from Vulkan SC. The test installs a custom `VkAllocationCallbacks` that tracks every allocation and can be told to fail one specific allocation index. It first measures how many allocations a successful instance+device creation requires, then retries creation while failing each allocation index in turn. Each failed attempt must return `VK_ERROR_OUT_OF_HOST_MEMORY` from `vkCreateInstance`, `vkEnumeratePhysicalDevices`, or `vkCreateDevice`, must leave the allocation tracker empty (no leaks), and must not return any other error code ([`createInstanceDeviceIntentionalAllocFail()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2580-L2781)). The source comment above the registration notes this test is excluded from Vulkan SC because in the SC main process it does not really create any instance or device and the creation calls always return `VK_SUCCESS`.
...
| `create_device_queue_priorities.basic` | Priority acceptance: a requested queue cannot be retrieved or idled successfully. No scheduling order is asserted. |
| `create_device_queue_priorities.concurrent_submission` | Concurrent queue submission or buffer isolation failure: a queue's completed fill does not match its sentinel. The test does not establish a priority scheduling guarantee. |
...
**Possible implementation causes:** The implementation allocates host memory during enumeration through the instance-provided allocator but does not free it before the call returns; the implementation routes some internal allocations outside the user-supplied `VkAllocationCallbacks` and then frees them later, breaking the recorder's accounting. Both would surface as a non-zero allocation balance in [`enumerateDevicesAllocLeakTest()`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L577-L626). Source-level investigation is needed to identify which allocations are leaked.
...
- Five intermediate nodes are compiled out for Vulkan SC builds through `#ifndef CTS_USES_VULKANSC` guards: `enumerate_devices_alloc_leak`, `create_device_global_priority_khr`, `create_device_global_priority_query`, `create_device_global_priority_query_khr`, and `create_instance_device_intentional_alloc_fail`. The source comment on the last node explains the exclusion: in the Vulkan SC main process, the test does not really create an instance or device, and the creation calls always return `VK_SUCCESS`, so the test would not exercise the intended path ([`vktApiDeviceInitializationTests.cpp`](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2899-L2899)).
...
- The 220 per-feature leaves under `create_device_unsupported_features` are generated from `vkDeviceFeatureTest.inl` rather than enumerated by hand. Each leaf targets exactly one feature bit; combinations of unsupported features are intentionally not tested here and belong to feature-interaction tests elsewhere.
...
- Most cases check creation configurations and queue usability; `create_device_queue_priorities.concurrent_submission` additionally checks buffer contents after GPU execution.
- The primary behavioral axis is the intermediate node. The 27 intermediate nodes group into nine thematic clusters: instance creation, enumeration leak, basic device creation, queue-priority behavior, global priority, features2, unsupported features, queue2 / protected memory, and intentional allocation failure.
- `create_device_queue_priorities` has two leaves: `basic` checks priority-valued queue creation and queue usability, while `concurrent_submission` checks independent host-visible buffer results after overlapping submissions. Neither leaf claims that queue priority controls scheduling order.
- `create_device_queue_priorities` and `create_device_unsupported_features` are the intermediate nodes with non-single-`basic` leaf shapes: queue priorities has `basic` plus `concurrent_submission`; unsupported features has `core` plus generated per-feature leaves.
...
|-------------|------|----------------|
| Public entry point: `createDeviceInitializationTests()` | [Public entry point: createDeviceInitializationTests()](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2868-L2899) | Builds the `device_init` test family and attaches all 26 intermediate nodes. |
| Wrapper helper: `addFunctionCaseInNewSubgroup()` | [Wrapper helper: addFunctionCaseInNewSubgroup()](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2839-L2866) | Creates each single-`basic` intermediate node. Used for 25 of the 26 direct children. |
| Parent registration: `createApiTests()` | [Parent registration: createApiTests()](../../../modules/vulkan/api/vktApiTests.cpp#L100) | Attaches `createDeviceInitializationTests()` to the `api` test category. |
| Instance creation tests | [Instance creation tests](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L66-L576) | Bodies for the six `create_instance_*` intermediate nodes. |
| `enumerateDevicesAllocLeakTest()` | [enumerateDevicesAllocLeakTest()](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L579-L628) | Allocation-balance check for `vkEnumeratePhysicalDevices`. |
| Basic device creation tests | [Basic device creation tests](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L629-L877) | Bodies for `create_device`, `create_multiple_devices`, `create_device_unsupported_extensions`, `create_device_various_queue_counts`. |
| Queue-priority tests | [Queue-priority tests](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L879-L1073) | Priority-valued queue creation and concurrent buffer-fill verification. |
| Global priority tests | [Global priority tests](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L1075-L1392) | Bodies and support hooks for the four `create_device_global_priority*` intermediate nodes. |
| `createDeviceFeatures2Test()` | [createDeviceFeatures2Test()](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L1318-L1383) | Features2 path body. |
| Unsupported-feature driver: `checkFeatures()` | [Unsupported-feature driver: checkFeatures()](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L1399-L1630) | Shared driver for the 220 per-feature leaves, including prerequisite-feature handling. |
| `createDeviceWithUnsupportedFeaturesTest()` | [createDeviceWithUnsupportedFeaturesTest()](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L1632-L1772) | `core` leaf body, enumerating `VkPhysicalDeviceFeatures`. |
| `addSeparateUnsupportedFeatureTests()` | [addSeparateUnsupportedFeatureTests()](../../../framework/vulkan/generated/vulkan/vkDeviceFeatureTest.inl#L8679-L8679) | Generator for the 220 per-feature leaves under `create_device_unsupported_features`. |
| Queue2 tests | [Queue2 tests](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L1776-L2444) | Bodies for the eight `create_device_queue2*` intermediate nodes and shared helpers. |
| `runQueueCreationTestCombination()` | [runQueueCreationTestCombination()](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2002-L2057) | Shared queue-retrieval and wait loop used by all protected-memory queue2 variants. |
| `createInstanceDeviceIntentionalAllocFail()` | [createInstanceDeviceIntentionalAllocFail()](../../../modules/vulkan/api/vktApiDeviceInitializationTests.cpp#L2618-L2833) | Intentional-allocation-failure body. |
