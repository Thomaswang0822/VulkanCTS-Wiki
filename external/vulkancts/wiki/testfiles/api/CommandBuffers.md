...
The two variables produce 64 direct leaves. The family is created by [`createCommandBuffersTests()`](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6266) and attached to the `api` test category by [Parent registration](../../../modules/vulkan/api/vktApiTests.cpp#L107).

## Parameter Dimensions and Observed Values

| Dimension | Registered values | Meaning in this test | Evidence |
|-----------|-------------------|----------------------|----------|
| Command buffer level | `VK_COMMAND_BUFFER_LEVEL_PRIMARY`, `VK_COMMAND_BUFFER_LEVEL_SECONDARY` | Selects which level the buffer is allocated and recorded at. Several leaves rerun the same scenario at both levels. | [Command buffer level](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6285-L6288) |
| Pool create flags | `0`, `VK_COMMAND_POOL_CREATE_TRANSIENT_BIT`, `VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT` | Each pool_create_* and several recording leaves exercise one flag value. The reset bit is required for any test that calls `vkResetCommandBuffer` on an individual buffer. | [Pool create flags](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6270-L6276) |
| Pool reset flags | `0`, `VK_COMMAND_POOL_RESET_RELEASE_RESOURCES_BIT` | Distinguishes "reset keeping resources" from "reset releasing resources" on the pool_reset_* leaves. The release bit is non-VulkanSC only. | [Pool reset flags](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6278-L6280) |
| Command buffer usage flags | `0`, `VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT`, `VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT`, `VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT` | Each recording behavioral group exercises one or a combination of these flags. | [Command buffer usage flags](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6327-L6334) |
| Framebuffer hint | `true`, `false` | Selects whether `VkCommandBufferInheritanceInfo::framebuffer` is `VK_NULL_HANDLE` for the render_pass_continue leaves. | [Framebuffer hint](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6334-L6338) |
| Buffer allocation count | `1`, `1024`, `10000` (100 on Vulkan SC) | Drives allocate_*_many leaves. The 32-bit path uses 1024 and the 64-bit path uses 10000 to probe driver limits. | [Buffer allocation count](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L607-L615) |
| Large buffer command count | `10000` (`1000` on Vulkan SC) | Drives record_many_* and record_large_secondary. Each iteration records a set/reset event pair. | [Large buffer command count](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L1164-L1168) |
| Bad inheritance info case | `RANDOM_PTR`, `RANDOM_PTR_CONTINUATION`, `RANDOM_DATA_PTR`, `INVALID_STRUCTURE_TYPE`, `VALID_NONSENSE_TYPE` | Each `bad_inheritance_info_*` leaf populates `VkCommandBufferBeginInfo::pInheritanceInfo` with one form of unused invalid data and checks that recording and execution still succeed. | [Bad inheritance info case](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6359-L6373) |
| State transition | `STT_RECORDING_TO_INITIAL`, `STT_EXECUTABLE_TO_INITIAL`, `STT_RECORDING_TO_INVALID`, `STT_EXECUTABLE_TO_INVALID` | Each `recording_to_*` / `executable_to_*` leaf drives one transition, then verifies the buffer is usable again after `vkResetCommandBuffer`. | [State transition](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L4358-L4364) |
| Image extent for many-draws | `128×128`, `512×256` | Drives the four `record_many_draws_*` leaves. The larger extent is non-VulkanSC only. | [Image extent for many-draws](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6309-L6325) |
| Submit count | `0`, `1`, `5` | Drives `submit_count_non_zero` (5), `submit_count_equal_zero` (0), and the per-buffer submit leaves (1). | [Submit count](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6375-L6377) |
| Indirect dispatch memory offset | `0, 4, 8, 12, 16, 20, 24, 28` | First axis of the 8×8 generated matrix. The indirect buffer is bound at this offset within an allocated device memory object. | [Indirect dispatch memory offset](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6418-L6428) |
| Indirect dispatch offset | `0, 4, 8, 12, 16, 20, 24, 28` | Second axis of the 8×8 generated matrix. The byte offset passed to `vkCmdDispatchIndirect` within the indirect buffer. | [Indirect dispatch offset](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6418-L6428) |

The 64 `indirect_compute_dispatch_offsets_{memOffset}_{dispatchOffset}` leaves are generated by a nested loop over the two offset axes above ([`createCommandBuffersTests()`](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6420-L6427)). Each leaf records a `vkCmdDispatchIndirect` on a secondary command buffer, with the indirect buffer bound at `memOffset` inside an allocation and the dispatch command read from `dispatchOffset` inside the indirect buffer. The host then verifies that each of the 64 storage buffer slots holds the expected `gl_LocalInvocationIndex + 1000000` value.
...
|---------------------|---------------------------------------|----------|
| Simultaneous-use case | `record_simul_use_secondary_one_primary`, `record_simul_use_secondary_two_primary`, `record_simul_use_nested`, and `record_simul_use_twice_nested` all register the same `genComputeIncrementSource()` shader; the shader declarations and atomic operation stay fixed while the host changes primary/secondary nesting and execution topology. | [Simultaneous-use case](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6340-L6351) |
| Dispatch count per executed secondary | The generated shader always performs one `atomicAdd` per one-invocation dispatch; the expected counter changes with how many times the host executes the secondary, not with shader source. | [Dispatch count per executed secondary](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L1893-L1923) |
| Program registration | The selected leaf uses `addFunctionCaseWithPrograms(..., genComputeIncrementSource, ...)`, so the primary reconstructed stage is the `compute_increment` GLSL source rather than `genComputeSource()` or an inline graphics stage. | [Program registration](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L4529-L4546) |
...
**Possible failure symptoms:** `simultaneousUse*` leaves read the atomic counter from the result buffer and find a value smaller than the expected number of dispatches (2 for every leaf).
...
- The non-null allocator, pool reset release-resources, pool reset reuse, trim, `record_many_draws_*_2`, `secondary_extras`, and `pipeline_shader_object_mix_with_secondaries` leaves are excluded on Vulkan SC through `#ifndef CTS_USES_VULKANSC` ([`SecondariesXferQueueCheckSupport()`](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6271-L6273), [`vktApiCommandBuffersTests.cpp`](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6277-L6283), [`SecondariesXferQueueRun()`](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6293-L6298), [`SecondariesXferQueueRun()`](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6317-L6325), [`createCommandBuffersTests()`](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6430-L6452)).
- Large iteration counts are reduced on Vulkan SC: `10000` command buffers becomes `100`, `5000` set/reset event pairs becomes `500`, and `10000` large-buffer events becomes `100` ([Buffer allocation count](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L607-L615), [`executeLargePrimaryBufferTest()`](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L769-L774), [Large buffer command count](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L1164-L1168)).
- The indirect dispatch alignment matrix uses fixed offset values `{0, 4, 8, 12, 16, 20, 24, 28}` and a fixed storage buffer slot count of 64; no other offset or slot combinations are generated ([`createCommandBuffersTests()`](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6418)).
- `submit_count_non_zero` uses a fixed `BUFFER_COUNT` of 5; `submit_count_equal_zero` uses 2; `secondary_execute_twice` uses 10 secondaries. These counts are not parameterized ([`main()`](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L2989), [`submitBufferCountNonZero()`](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L3081), [`executeMultipleLevelsNestedBufferTest()`](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L3980)).
- The `record_many_draws_*` leaves use two fixed image extents (`128×128` and `512×256`) rather than a generated extent matrix ([Image extent for many-draws](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6309-L6325)).
...
|-------------|------|----------------|
| `createCommandBuffersTests()` | [createCommandBuffersTests()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6266) | Top-level registration for the `command_buffers` test family; adds 131 direct leaves, including the 64-case generated matrix. |
| Parent registration | [Parent registration](../../../modules/vulkan/api/vktApiTests.cpp#L107) | `apiTests->addChild(createCommandBuffersTests(testCtx))` attaches the family to the `api` test category. |
| `createPoolNullParamsTest()` | [createPoolNullParamsTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L380) | Pool creation leaves. |
| `resetPoolReuseTest()` | [resetPoolReuseTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L511) | Pool reset and reuse leaf. |
| `trimCommandPoolTest()` | [trimCommandPoolTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L999) | Trim leaves, parameterized by command buffer level. |
| `allocatePrimaryBufferTest()` | [allocatePrimaryBufferTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L564) | Allocation leaves. |
| `executePrimaryBufferTest()` | [executePrimaryBufferTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L712) | Primary buffer execution via event signaling. |
| `resetBufferImplicitlyTest()` | [resetBufferImplicitlyTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L830) | Implicit reset by re-recording. |
| `recordSinglePrimaryBufferTest()` | [recordSinglePrimaryBufferTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L1088) | Recording leaves. |
| `recordLargeSecondaryBufferTest()` | [recordLargeSecondaryBufferTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L1241) | Large secondary recording with embedded `cmdExecuteCommands`. |
| `submitPrimaryBufferTwiceTest()` | [submitPrimaryBufferTwiceTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L1337) | Submit-twice leaves. |
| `oneTimeSubmitFlagPrimaryBufferTest()` | [oneTimeSubmitFlagPrimaryBufferTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L1519) | One-time submit leaves. |
| `renderPassContinueTest()` | [renderPassContinueTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L1725) | Render pass continue leaves, parameterized by framebuffer hint. |
| `renderPassContinueNestedTest()` | [renderPassContinueNestedTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L1926) | Nested render pass continue leaf. |
| `simultaneousUseSecondaryBufferOnePrimaryBufferTest()` | [simultaneousUseSecondaryBufferOnePrimaryBufferTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L1778) | Simultaneous-use leaves. |
| `badInheritanceInfoTest()` | [badInheritanceInfoTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L2371) | Bad inheritance info leaves, parameterized by `BadInheritanceInfoCase`. |
| `recordBufferQueryPreciseWithFlagTest()` | [recordBufferQueryPreciseWithFlagTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L2714) | Query recording leaves. |
| `submitBufferCountNonZero()` | [submitBufferCountNonZero()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L2982) | Submission count leaves. |
| `submitBufferCountEqualZero()` | [submitBufferCountEqualZero()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L3074) | Zero-count submit leaf; inverts the event check. |
| `submitBufferWaitSingleSemaphore()` | [submitBufferWaitSingleSemaphore()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L3178) | Semaphore wait leaves. |
| `submitBufferNullFence()` | [submitBufferNullFence()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L3451) | Null-fence submit leaf. |
| `submitTwoBuffersOneBufferNullWithFence()` | [submitTwoBuffersOneBufferNullWithFence()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L3555) | Null-buffer-with-fence submit leaf. |
| `executeSecondaryBufferTest()` | [executeSecondaryBufferTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L3647) | Secondary execution leaf. |
| `executeNestedBufferTest()` | [executeNestedBufferTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L3741) | Nested execution leaf. |
| `executeMultipleLevelsNestedBufferTest()` | [executeMultipleLevelsNestedBufferTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L3871) | Multi-level nested execution leaf. |
| `executeSecondaryBufferTwiceTest()` | [executeSecondaryBufferTwiceTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L3978) | Secondary execute twice leaf. |
| `orderBindPipelineTest()` | [orderBindPipelineTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L4144) | Pipeline binding order leaf. |
| `executeStateTransitionTest()` | [executeStateTransitionTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L4366) | State transition leaves, parameterized by `StateTransitionTest`. |
| `genComputeSource()` | [genComputeSource()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L4471) | Compute source for `order_bind_pipeline` (good and bad variants). |
| `genComputeIncrementSource()` | [genComputeIncrementSource()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L4529) | Atomic-counter compute source for simultaneous-use and bad-inheritance leaves. |
| `ManyDrawsCase` class | [ManyDrawsCase class](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L4713) | `record_many_draws_*` test case class. |
| `manyIndirectDrawsTest()` | [manyIndirectDrawsTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L5313) | Many indirect draws leaf. |
| `manyIndirectDispatchesTest()` | [manyIndirectDispatchesTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L5458) | Many indirect dispatches leaf. |
| `IndirectDispatchAlignmentCase` class | [IndirectDispatchAlignmentCase class](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L5592) | 64-leaf generated matrix for `vkCmdDispatchIndirect` alignment. |
| `secCmdExtraCaseTest()` | [secCmdExtraCaseTest()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L5802) | Secondary extras leaves. |
| `PipelineShaderObjMixSecondariesRun()` | [PipelineShaderObjMixSecondariesRun()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6022) | Pipeline and shader object mix leaf. |
| `SecondariesXferQueueRun()` | [SecondariesXferQueueRun()](../../../modules/vulkan/api/vktApiCommandBuffersTests.cpp#L6199) | Secondary on transfer queue leaf. |
| Header | [Header](../../../modules/vulkan/api/vktApiCommandBuffersTests.hpp#L1) | Declares `createCommandBuffersTests()`. |
