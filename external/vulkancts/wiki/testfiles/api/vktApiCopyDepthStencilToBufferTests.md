...
Evidence:
- `addCopyDepthStencilToBufferTests` at [line 577](../../../modules/vulkan/api/vktApiCopyDepthStencilToBufferTests.cpp#L577) is called from [vktApiCopiesAndBlittingTests.cpp](../../../modules/vulkan/api/vktApiCopiesAndBlittingTests.cpp#L1) under multiple branches of `copy_and_blit`
- Under `core`: `depthstencil_to_buffer` at [line 134](../../../modules/vulkan/api/vktApiCopiesAndBlittingTests.cpp#L134), `depthstencil_to_buffer_compute_queue` at [line 150](../../../modules/vulkan/api/vktApiCopiesAndBlittingTests.cpp#L150), `depthstencil_to_buffer_transfer_queue` at [line 163](../../../modules/vulkan/api/vktApiCopiesAndBlittingTests.cpp#L163)
- The same three groups also appear under `dedicated_allocation` and `copy_commands2`
- The hierarchy tree above shows the `core` branch as the canonical root; the same test groups are replicated under `dedicated_allocation` and `copy_commands2` with different `TestGroupParams` (allocation kind, extension flags, queue selection)

## Test Families

### depthstencil_to_buffer — Copy depth/stencil image to buffer (universal queue)

Registered at [line 134](../../../modules/vulkan/api/vktApiCopiesAndBlittingTests.cpp#L134) under `core`. Uses `CopyDepthStencilToBuffer` test instance inheriting from `CopiesAndBlittingTestInstanceWithSparseSemaphore` ([line 37](../../../modules/vulkan/api/vktApiCopyDepthStencilToBufferTests.cpp#L37)).

Creates a source depth/stencil image and destination buffer, uploads known depth/stencil data to the source image, issues copy commands, reads back the buffer, and verifies against a software reference.

Test case naming pattern (generated by `addCopyDepthStencilToBufferTests` at [line 577](../../../modules/vulkan/api/vktApiCopyDepthStencilToBufferTests.cpp#L577)):
...
- `buffer_offset_<format>_{DS,D_S,SD,SD_combined,D,S}` — same aspects with buffer offset

Under `core`, the `extensionFlags` parameter is NONE (standard `vkCmdCopyImageToBuffer`).
...
### depthstencil_to_buffer_compute_queue — Copy depth/stencil image to buffer (compute-only queue)

Registered at [line 150](../../../modules/vulkan/api/vktApiCopiesAndBlittingTests.cpp#L150) under `core`. Same test instance and case as `depthstencil_to_buffer`, but with `QueueSelectionOptions::ComputeOnly` and `MAINTENANCE_10` extension flag. Requires `VK_FORMAT_FEATURE_2_DEPTH_COPY_ON_COMPUTE_QUEUE_BIT_KHR` / `STENCIL_COPY_ON_COMPUTE_QUEUE_BIT_KHR` ([line 519](../../../modules/vulkan/api/vktApiCopyDepthStencilToBufferTests.cpp#L519)).

### depthstencil_to_buffer_transfer_queue — Copy depth/stencil image to buffer (transfer-only queue)

Registered at [line 163](../../../modules/vulkan/api/vktApiCopiesAndBlittingTests.cpp#L163) under `core`. Same test instance and case as `depthstencil_to_buffer`, but with `QueueSelectionOptions::TransferOnly` and `MAINTENANCE_10` extension flag. Requires `VK_FORMAT_FEATURE_2_DEPTH_COPY_ON_TRANSFER_QUEUE_BIT_KHR` / `STENCIL_COPY_ON_TRANSFER_QUEUE_BIT_KHR` ([line 547](../../../modules/vulkan/api/vktApiCopyDepthStencilToBufferTests.cpp#L547)).
