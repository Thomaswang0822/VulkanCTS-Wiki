...
| useSparseBinding | `false`, `true` | When true, the source image is created with sparse binding and residency flags, bound through `vkQueueBindSparse`, and synchronized with a sparse semaphore before the copy. Currently routed through the `copy_and_blit.sparse` dispatcher for related image-to-image tests; this family does not register sparse leaves directly under `core`. | [`vktApiCopyDepthStencilToBufferTests.cpp#L137-L166`](../../../modules/vulkan/api/vktApiCopyDepthStencilToBufferTests.cpp#L137-L166) |
