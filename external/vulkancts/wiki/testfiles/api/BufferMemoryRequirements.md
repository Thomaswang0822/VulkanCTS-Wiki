...
- **VUID-None-01888 enforcement:** any combination that mixes `protected` with a sparse bit has the sparse bits removed, leaving only the `protected` bit; the subsequent duplicate-removal step then collapses these into the standalone `{protected}` entry ([L211-L226](../../../modules/vulkan/api/vktApiBufferMemoryRequirementsTests.cpp#L211-L226), [L241-L245](../../../modules/vulkan/api/vktApiBufferMemoryRequirementsTests.cpp#L241-L245)). This is why no `create_protected_sparse_*` intermediate node exists.
