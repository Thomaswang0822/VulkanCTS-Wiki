...
- The family is registered for non-Vulkan SC builds only; the call at [register fragment-output tests outside Vulkan SC](../../../modules/vulkan/api/vktApiTests.cpp#L134) is guarded by `#ifndef CTS_USES_VULKANSC` (lines 128-137), and the family is absent from the `vksc-default` mustpass.
...
`createFragmentShaderOutputTests` creates the `fragment_shader_output` test family and is attached to the `api` test category by [attach fragment-output tests to the API category](../../../modules/vulkan/api/vktApiTests.cpp#L134). The three intermediate nodes are registered from the local `cases[]` table at [map output-mismatch nodes to case types](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L706-L715); the same loop also drives the per-node leaf generation at [enumerate signedness pairs and attachment permutations](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L730-L768).
...
Each leaf has exactly two color attachments. The (shaderFormat, renderFormat) pair for each attachment is drawn from `signednessFormats`, a list built at [filter shader/render format pairs by integer class](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L717-L726) by keeping only pairs whose two formats share the same integer/non-integer class: both `R8_UINT`/`R8_SINT` (integer) or both `R8_UNORM`/`R8_SNORM` (non-integer). The registration loop at [combine two format pairs without repeating formats](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L735-L749) combines two such pairs into a single test. The two pairs must differ in both shader format and render format, so no two attachments repeat the same shader format or the same render format.
...
Verification requires every attachment to be rendered with the shader-written value. The check is performed by `isBufferRendered` at [check shader-written values for every signedness attachment](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L578-L583). The expected value follows bit-pattern preservation for integer attachments: a `sint` shader output (`111`) read back from a `uint` attachment must equal `111`, and a `uint` shader output (`123`) read back from a `sint` attachment must equal `123`. For `unorm`/`snorm` attachments, the expected value is `1.0f`.
...
```text
dEQP-VK.api.fragment_shader_output.attachment_no_location.sint2sint_snorm2snorm_uint2uint_unorm2unorm
```

| Parameter choice | Meaning in this representative case |
|------------------|-------------------------------------|
| `attachment_no_location` | The render pass has four color attachments, but the fragment shader deliberately emits no output at `magicLoc = attachments / 2 = 2`; attachment 2 must therefore remain at its clear value. |
| `sint2sint_snorm2snorm_uint2uint_unorm2unorm` | The four attachment slots use matching shader/render formats in the order `R8_SINT`, `R8_SNORM`, `R8_UINT`, `R8_UNORM`. This selects `ivec4`/`111` at location 0, `vec4`/`1.0` at location 1, an omitted `uvec4`/`123` at location 2, and `vec4`/`1.0` at location 3. |

#### Purpose

This representative verifies that an attachment with no matching fragment output location is not overwritten: the output interface leaves attachment 2 at its clear value while locations 0, 1, and 3 write their typed constants to the corresponding attachments.

#### Structural Design

| Fragment output | Shader declaration/store | Expected attachment result |
|-----------------|--------------------------|-----------------------------|
| Location 0 | `layout(location = 0) out ivec4 color0`; store four `111` values | Attachment 0 contains the signed shader value |
| Location 1 | `layout(location = 1) out vec4 color1`; store four `1.0` values | Attachment 1 contains normalized `1.0` |
| Location 2 | No output declaration and no store | Attachment 2 remains at its `loadOp = CLEAR` value |
| Location 3 | `layout(location = 3) out uvec4 color3`; store four `123` values | Attachment 3 contains the unsigned shader value |
...
/// Attachment 0 uses the signed integer shader output type and receives the signed test constant.
layout(location = 0) out ivec4 color0;
/// Attachment 1 uses a normalized floating-point shader output and receives the saturated constant.
layout(location = 1) out vec4 color1;
/// Location 2 is intentionally omitted: the render pass still has attachment 2, which must retain its clear value.
/// Attachment 3 uses a normalized floating-point shader output and receives the saturated constant.
layout(location = 3) out vec4 color3;

void main()
{
    /// This store should render to attachment 0.
    color0 = ivec4(111, 111, 111, 111);
    /// This store should render to attachment 1 as normalized value 1.0.
    color1 = vec4(1.0, 1.0, 1.0, 1.0);
    /// There is no color2 store because the representative omits output location 2.
    /// This store should render to attachment 3 as normalized value 1.0.
...
- `initPrograms()` computes `magicLoc` as `attachments / 2`, so this four-attachment case omits location 2 while retaining declarations and stores for locations 0, 1, and 3 ([omit the middle fragment output location](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L324-L354)).
- The host verifier checks the omitted slot with `isBufferUnchanged` and all other slots with `isBufferRendered`; the clear value is not a fixed generic constant but is generated per render format by `makeClearColors()` ([generate clear colors for each attachment format](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L431-L459), [compare unchanged colors and rendered values](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L475-L533)).
- The selected leaf is one of the permutation-generated four-attachment cases; its shader and render format for each slot are identical, while the `different_signedness` branch separately exercises same-integer-class shader/render mismatches ([restrict signedness pairs to matching integer classes](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L717-L726), [permute four matching shader/render attachment formats](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L751-L766)).

#### Parameter Variation Summary

| Parameter dimension | Shader-level variation from this shader | Evidence |
|---------------------|-----------------------------------------|----------|
| `attachment_no_location` vs. `location_no_attachment` | The former omits the `magicLoc` declaration and store; the latter emits that value at `location = attachments`, beyond the four declared attachments. | [initPrograms](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L324-L354) |
| Format permutation for the mismatch branches | Changes each output's GLSL type and constant: integer formats select `ivec4`/`uvec4` with `111`/`123`, while normalized formats select `vec4` with `1.0`. | [type/value lambdas](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L295-L322) |
| `different_signedness` | Uses two outputs and pairs shader/render formats within the same integer/non-integer class; declarations and typed stores vary with each pair. | [signednessFormats and registration](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L717-L749) |
...
; Schema: 0
          ...
          %1 = OpExtInstImport "GLSL.std.450"
               ...
               OpEntryPoint Fragment %main "main" %color0 %color1 %color3
               ...
               OpDecorate %color3 Location 3
       %void = OpTypeVoid
          %3 = OpTypeFunction %void
        %int = OpTypeInt 32 1
      %v4int = OpTypeVector %int 4
%_ptr_Output_v4int = OpTypePointer Output %v4int
     %color0 = OpVariable %_ptr_Output_v4int Output
    %int_111 = OpConstant %int 111
         %11 = OpConstantComposite %v4int %int_111 %int_111 %int_111 %int_111
...
%_ptr_Output_v4float = OpTypePointer Output %v4float
    ...
    %float_1 = OpConstant %float 1
     ...
     %color3 = OpVariable %_ptr_Output_v4float Output
...
**Possible failure symptoms:** In a `different_signedness` leaf with a `unorm` or `snorm` render attachment, the readback pixel is not exactly `1.0f`. `isBufferRendered` compares the float channel against `1.0f` with exact equality at [require exact unity for normalized rendered values](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L528); the `0.001f` tolerance in `isBufferUnchanged` applies only to the clear-color check, not to the rendered-value check.
...
- The `location_no_attachment` and `attachment_no_location` branches walk only the permutations returned by `std::next_permutation` starting from the sorted `formatsWithNames` vector. The initial sorted permutation is not emitted as a leaf because the `while` loop body only runs after the first `next_permutation` call returns true. The result is 23 leaves out of the 24 possible permutations of four distinct formats. See [register permutations after the initial sorted arrangement](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L753-L766).
- The `different_signedness` branch restricts each attachment's (shaderFormat, renderFormat) pair to same-class combinations through `signednessFormats`. Cross-class pairs (for example `sint` shader to `unorm` render) are intentionally excluded from the matrix; this is a design choice, not a hardware requirement. See [exclude cross-class shader/render format pairs](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L717-L726).
- The `different_signedness` registration loop skips any pair of `signednessFormats` entries that share a shader format or a render format, so no leaf repeats the same shader format or the same render format across its two attachments. See [skip pairs that reuse either shader or render format](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L738-L740).
- The shader format and render format are always identical for the `location_no_attachment` and `attachment_no_location` branches; the permutation varies only the assignment of formats to attachment slots. See [keep shader and render formats identical in permutation cases](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L760-L763).
...
|-------------|------|----------------|
| Parent registration in `createApiTests` | [attach fragment-output tests to the API category](../../../modules/vulkan/api/vktApiTests.cpp#L134) | Attaches `fragment_shader_output` to the `api` test category. |
| Family factory `createFragmentShaderOutputTests` | [build output-mismatch groups and format combinations](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L698-L772) | Builds the root group, the format matrix, the signedness-class filter, and the per-node leaf registration loops. |
| `formatsWithNames` declaration | [classify the four R8 formats as integer or normalized](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L700-L705) | The four `R8` formats and their integer/non-integer classification flag. |
| `cases[]` table | [map output-mismatch nodes to case types](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L706-L715) | Maps each intermediate node name to its `ShaderOutputCases` enum and `signedness` flag. |
| `signednessFormats` build | [filter shader/render format pairs by integer class](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L717-L726) | Same-class (shader/render) pair filter used by the `different_signedness` branch. |
| `different_signedness` registration loop | [combine two format pairs without repeating formats](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L735-L749) | Combines two same-class pairs into one two-attachment leaf, skipping shared shader or render formats. |
| Permutation registration loop | [register permutations after the initial sorted arrangement](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L753-L766) | Generates the 23 leaves for each of `location_no_attachment` and `attachment_no_location`. |
| Shader generation `initPrograms` | [generate fragment outputs with a rerouted or omitted location](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L291-L368) | Builds the fragment shader, including the `magicLoc` reroute (`LocationNoAttachment`) and the `magicLoc` skip (`AttachmentNoLocation`). |
| `magicLoc` computation | [choose the middle attachment as the unmatched location](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L324) | `attachments / 2` selects the unmatched location index. |
| Render pass creation `createColorRenderPass` | [configure clear, store, and transfer-readback attachment operations](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L116-L172) | Builds the multi-attachment render pass with `loadOp = CLEAR`, `storeOp = STORE`, and `finalLayout = TRANSFER_SRC_OPTIMAL`. |
| Pipeline creation `createGraphicsPipeline` | [create a color-blend state for each attachment](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L174-L194) | Builds the graphics pipeline with one `VkPipelineColorBlendAttachmentState` per attachment. |
| Support checks `checkSupport` | [check attachment limits and color/transfer format usage](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L245-L289) | Enforces `maxColorAttachments` and per-format `COLOR_ATTACHMENT_BIT | TRANSFER_SRC_BIT` support. |
| `makeClearColors` | [generate clear colors for each attachment format](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L431-L459) | Builds per-attachment clear colors used by `isBufferUnchanged`. |
| `verifyResults` | [select clear-color or rendered-value checks per output case](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L465-L589) | Host-side pixel scan; dispatches to `isBufferUnchanged`/`isBufferRendered` per intermediate node. |
| `isBufferUnchanged` and `isBufferRendered` | [compare unchanged colors and rendered values](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L475-L533) | Per-attachment expected-value comparisons for clear-color and shader-written cases. |
| `iterate` | [set up, draw, copy back, and verify fragment outputs](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L591-L666) | End-to-end host sequence: setup, draw, copy-back, verify, pass/fail return. |
| `TestConfig` and `ShaderOutputCases` | [carry output case type and per-attachment formats](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L57-L84) | Configuration struct and enum that carry the per-leaf case type and format list. |
