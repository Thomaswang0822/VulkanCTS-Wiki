...
- `initPrograms()` computes `magicLoc` as `attachments / 2`, so this four-attachment case omits location 2 while retaining declarations and stores for locations 0, 1, and 3 ([source](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L324-L354)).
- The host verifier checks the omitted slot with `isBufferUnchanged` and all other slots with `isBufferRendered`; the clear value is not a fixed generic constant but is generated per render format by `makeClearColors()` ([source](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L431-L459), [source](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L475-L533)).
- The selected leaf is one of the permutation-generated four-attachment cases; its shader and render format for each slot are identical, while the `different_signedness` branch separately exercises same-integer-class shader/render mismatches ([source](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L717-L726), [source](../../../modules/vulkan/api/vktApiFragmentShaderOutputTests.cpp#L751-L766)).
