...
The `buffer_marker` test family is created by [`createBufferMarkerTestsInGroup()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1083-L1248), which is exposed to the `api` test category through [`createBufferMarkerTests()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1252-L1255).

## Parameter Dimensions and Observed Values

| Dimension | Registered values | Meaning in this test | Evidence |
|-----------|-------------------|----------------------|----------|
| Queue type | `graphics`, `compute`, `transfer` | Targets universal, compute-only, or transfer-only queue families via `makeQueueCreateInfo` with `forbiddenFlags`. Determines which `memory_dep` methods are legal. | [Queue type](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L69-L92), [Queue type](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1086-L1087) |
| Memory type | `external_host_mem`, `default_mem` | `external_host_mem` imports a host pointer through `VK_EXT_external_memory_host`; `default_mem` uses the standard allocator. Both backings are host-visible so the result can be invalidated and read. | [Memory type](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1097-L1098), [Memory type](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L206-L246) |
| Pipeline stage | `top_of_pipe`, `bottom_of_pipe` | The `stage` parameter passed to `vkCmdWriteBufferMarkerAMD`. Selects when in the pipeline the marker write occurs. | [Pipeline stage](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1108-L1110) |
| Test type | `sequential`, `overwrite`, `memory_dep` | The primary behavioral axis. Each test type runs a different test function with its own write pattern and verification rule. | [Test type](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1118-L1234) |
| `memory_dep` method | `draw`, `dispatch`, `buffer_copy` | The non-marker writer interleaved with marker writes. `draw` is registered only on `graphics`; `dispatch` on `graphics` and `compute`; `buffer_copy` on all three queue types. | [memorydep method](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1199-L1231) |
| Marker buffer size | `1`, `4`, `64`, `128`, `65536` | Number of 32-bit slots in the marker buffer. `128` is fixed for `memory_dep`; `1`, `4`, `64`, `65536` cover the `sequential` and `overwrite` ranges. | [Marker buffer size](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1121-L1154), [Marker buffer size](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1163-L1185), [Marker buffer size](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1202-L1203) |
| Buffer offset | `0`, `16`, `24`, `1024` | Non-zero offsets exercise alignment, non-coherent atom size, and external host pointer alignment requirements. `16` pairs with size `64`; `24` with size `64` (overwrite) and size `128` (`memory_dep`); `1024` with size `65536`. | [Buffer offset](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1121-L1154), [Buffer offset](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1180-L1185), [Buffer offset](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1196-L1197) |
| `memory_dep` iterations | `1000` | Fixed iteration count for the random ownership-alternation loop. | [memorydep iterations](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L493) |
...
Writes `params.size` marker values into consecutive 32-bit slots in submission order, then verifies the host-visible buffer contains exactly those values. The test uses a deterministic `de::Random` seeded with `12345 ^ params.size` so the expected values are reproducible. A single `VkMemoryBarrier` with `VK_ACCESS_TRANSFER_WRITE_BIT` → `VK_ACCESS_HOST_READ_BIT` separates the marker writes from host readback. Implemented in [`bufferMarkerSequential()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L249-L320).

### `overwrite` — random in-place marker overwrites

Performs `params.size * 10` random marker writes, each picking a slot and writing the iteration index as the value. The expected buffer is the last value written to each slot. This stresses the implementation's handling of repeated writes to the same slot from the same command buffer. Uses the same final memory barrier as `sequential`. Implemented in [`bufferMarkerOverwrite()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L322-L396).

### `memory_dep` — marker writes interleaved with non-marker writes

For 1000 iterations, randomly picks a slot and a new owner (`MEMORY_DEP_OWNER_MARKER` or `MEMORY_DEP_OWNER_NON_MARKER`). When ownership of the slot changes from the previous owner (excluding the first write to a previously untouched slot), or when the previous and new owners are both `MEMORY_DEP_OWNER_NON_MARKER`, the test inserts a `VkBufferMemoryBarrier` computed by [`computeMemoryDepBarrier()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L454-L482), then performs either a marker write or a non-marker write. Consecutive `MEMORY_DEP_OWNER_MARKER` writes to the same slot do not trigger a barrier. The final barrier accumulates `writeStages` and `writeAccess` actually used and transfers them to `VK_ACCESS_HOST_READ_BIT`. Implemented in [`bufferMarkerMemoryDep()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L486-L1001), with the barrier condition at [`bufferMarkerMemoryDep()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L886-L908).
...
- `initMemoryDepPrograms()` emits this compute source only for `MEMORY_DEP_DISPATCH`; the builder registers it for `compute` and `graphics` queues, but not `transfer` ([Memory type](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1001-L1047), [Queue type](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1215-L1223)).
- The selected registration fixes `size = 128` and uses offset `0`; `bufferMarkerMemoryDep()` performs 1000 random slot/owner iterations and binds the storage buffer at descriptor binding 0 ([`BufferMarkerMemDepCase()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L484-L513), [Buffer offset](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1193-L1203)).
- For this dispatch variant, `computeMemoryDepBarrier()` maps a non-marker owner to `VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT` and `VK_ACCESS_SHADER_WRITE_BIT`; marker ownership maps to `params.base.stage | VK_PIPELINE_STAGE_TRANSFER_BIT` and `VK_ACCESS_TRANSFER_WRITE_BIT` ([`BufferMarkerMemDepCase()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L452-L480)).

#### Parameter Variation Summary

| Parameter dimension | Shader-level variation from this shader | Evidence |
|---------------------|-----------------------------------------|----------|
| Queue type | `dispatch` is present for `graphics` and `compute`; `transfer` omits this shader method. | [Queue type](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1215-L1223) |
| Memory type | `default_mem` versus `external_host_mem` changes host allocation/import and coherency handling, not the generated compute GLSL. | [Memory type](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1097-L1106), [Memory type](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1001-L1047) |
| Marker stage | `top_of_pipe` versus `bottom_of_pipe` changes marker producer stage and barrier masks; the compute shader remains unchanged. | [Marker stage](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L457-L472), [Marker stage](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1108-L1116) |
| Memory-dependency method | `draw` uses a fragment shader, `buffer_copy` uses `vkCmdUpdateBuffer` and no shader, while `dispatch` uses this compute shader. | [Memory-dependency method](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1001-L1047) |
| Buffer offset | `0` versus `24` changes the bound allocation offset; the generated compute source and logical slot indexing remain unchanged. | [Buffer offset](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1193-L1203), [Buffer offset](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L508-L510) |
...
The required-device capability cache key includes queue type, allocation offset, and `useHostPtr`. This keeps imported-host-memory cases from reusing a device configuration prepared for ordinary allocation ([key](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L56-L64)).
...
**Possible failure symptoms:** `checkMarkerBuffer` finds at least one slot in the marker buffer that does not match the expected value from the `expected` vector built by the host, in a `sequential` or `overwrite` case where no non-marker write ever touches the buffer [`BufferMarkerBaseCase()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L152-L173), [`ExternalHostAllocation()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L248-L319).
...
**Possible failure symptoms:** `checkMarkerBuffer` reports a mismatch in an `overwrite` case, where the failing slot does not contain the value of the last `vkCmdWriteBufferMarkerAMD` call recorded for that slot [overwrite test function](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L321-L395).
...
**Possible failure symptoms:** `checkMarkerBuffer` reports a mismatch in a `memory_dep` case, where the failing slot contains the value from the wrong writer (marker instead of non-marker, or vice versa) for the last recorded operation on that slot [`BufferMarkerMemDepCase()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L484-L999), [`bufferMarkerMemoryDep()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L877-L978).

**Possible implementation causes:** The test inserts a `VkBufferMemoryBarrier` when slot ownership changes between marker and non-marker writers, or when consecutive non-marker writers target the same slot, with source and destination access masks and stage masks computed by [`computeMemoryDepBarrier()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L454-L482). A failure points to the implementation not honoring the barrier's availability/visibility operations across the two write types, executing the new write before the prior write completes, or applying the wrong stage or access mask when relating marker writes (reported as `VK_ACCESS_TRANSFER_WRITE_BIT` at `params.stage | VK_PIPELINE_STAGE_TRANSFER_BIT`) to non-marker writes. The `draw` and `dispatch` variants additionally depend on shader-side storage-buffer stores becoming visible at the expected stage; a shader compiler or storage-buffer access defect could also produce this symptom, and source-level investigation is needed to distinguish synchronization faults from shader-store faults.

#### External host memory import or coherency defect

**Possible failure symptoms:** A failure appears only on `external_host_mem` variants while the corresponding `default_mem` variant passes, with `checkMarkerBuffer` reading stale or partial marker values after invalidation [`BufferMarkerBaseCase()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L143-L173), [`ExternalHostAllocation()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L206-L246).
...
**Possible failure symptoms:** A failure appears only on variants with non-zero `offset` (`16`, `24`, `1024`) while the corresponding zero-offset variant passes [`main()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1135-L1154), [Buffer offset](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1180-L1185), [`createBufferMarkerTestsInGroup()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1196-L1203).
...
- `VK_AMD_buffer_marker` is required for every case; the device must support the extension or the case is unsupported [`main()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1057).
- `VK_EXT_external_memory_host` is required when `useHostPtr` is true, so `external_host_mem` variants are skipped on implementations without that extension [`main()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1054-L1055).
- `DEVICE_CORE_FEATURE_FRAGMENT_STORES_AND_ATOMICS` is required for the `memory_dep` `draw` method because the fragment shader performs a storage-buffer store [`main()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1067-L1071).
- The `draw` method is registered only on `graphics` because draw operations require a graphics queue [`createBufferMarkerTestsInGroup()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1205-L1213).
- The `dispatch` method is registered on `graphics` and `compute` and skipped on `transfer` because compute dispatch requires a compute-capable queue [Queue type](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1215-L1223).
- The `buffer_copy` method is registered on all three queue types because `vkCmdUpdateBuffer` is a transfer operation [`createBufferMarkerTestsInGroup()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1225-L1230).

### Design-based pruning

- The matrix is not exhaustive. Only selected `(size, offset)` pairs are registered for `sequential` (`4/0`, `64/0`, `64/16`, `65536/0`, `65536/1024`) and `overwrite` (`1/0`, `4/0`, `64/0`, `64/24`) [`main()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1118-L1190).
- `memory_dep` fixes `size = 128` and registers only `offset` values `0` and `24` [`createBufferMarkerTestsInGroup()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1196-L1203).
- The pipeline-stage dimension is limited to `top_of_pipe` and `bottom_of_pipe`; other stages are not registered.
- The `memory_dep` iteration count is fixed at `1000`; smaller or larger stress counts are not registered [`BufferMarkerMemDepCase()`](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L493).
...
- The `memory_dep` design isolates synchronization correctness: barriers are inserted when slot ownership changes or when consecutive non-marker writers target the same slot, with stages and access masks computed from the writer types, so a failure points directly to barrier handling rather than to marker write mechanics.
...
|-------------|------|----------------|
| Test category attachment | [Test category attachment](../../../modules/vulkan/api/vktApiTests.cpp#L103-L105) | Adds `buffer_marker` to the `api` test category under `#ifndef CTS_USES_VULKANSC`. |
| Factory declaration | [Factory declaration](../../../modules/vulkan/api/vktApiBufferMarkerTests.hpp#L34) | Declares `createBufferMarkerTests`. |
| Test family creation | [Test family creation](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1081-L1246) | Builds the `buffer_marker` tree with queue, memory, stage, and test-type intermediate nodes. |
| Queue selection helper | [Queue selection helper](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L69-L92) | `makeQueueCreateInfo` chooses the most specific queue family for each `VkQueueFlagBits`. |
| Test case wrapper | [Test case wrapper](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L109-L129) | `BufferMarkerBaseCase` configures device queues, extensions, and allocator offset parameters per case. |
| `memory_dep` wrapper | [memorydep wrapper](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L424-L450) | `BufferMarkerMemDepCase` extends the wrapper to add the `fragmentStoresAndAtomics` feature for `memory_dep`. |
| Marker buffer memory setup | [Marker buffer memory setup](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L206-L246) | `createMarkerBufferMemory` allocates or imports host memory and binds the buffer. |
| `sequential` test function | [sequential test function](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L248-L319) | Records ordered marker writes and verifies the buffer. |
| `overwrite` test function | [overwrite test function](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L321-L395) | Records random repeated marker writes and verifies the last-write value per slot. |
| `memory_dep` test function | [memorydep test function](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L484-L999) | Records 1000 iterations of marker or non-marker writes with ownership-change barriers. |
| `computeMemoryDepBarrier` | [computeMemoryDepBarrier](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L452-L480) | Computes source/destination access masks and stage masks for each writer type. |
| Result check | [Result check](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L152-L173) | `checkMarkerBuffer` invalidates memory and compares each slot against the expected vector. |
| Support checks | [Support checks](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1052-L1072) | Requires the extensions and core features needed for each case. |
| Shader generation for `memory_dep` | [Shader generation for memorydep](../../../modules/vulkan/api/vktApiBufferMarkerTests.cpp#L1001-L1050) | Emits the trivial vertex/fragment or compute shader used as the non-marker writer. |
| Mustpass entry range | [Mustpass entry range](../../../mustpass/main/vk-default/api.txt#L3077-L3232) | Lists all 156 `dEQP-VK.api.buffer_marker.*` test case leaves. |
