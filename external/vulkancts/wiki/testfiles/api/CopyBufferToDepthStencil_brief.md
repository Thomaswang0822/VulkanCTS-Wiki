...
| `addCopyBufferToDepthStencilTests`: registration loop and six-case matrix | [vktApiCopyBufferToDepthStencilTests.cpp#L742-L878](../../../modules/vulkan/api/vktApiCopyBufferToDepthStencilTests.cpp#L742-L852) | Generates the six `_DS`/`_D_S`/`_S_D`/`_SD`/`_D`/`_S` leaves for every depth/stencil format and offset mode |
