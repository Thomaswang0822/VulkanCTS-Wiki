...
The default mustpass scope contains 4,288 leaves under the monolithic root, 4,296 under `pipeline.fast_linked_library`, and 1,528 under `pipeline.shader_object_unlinked_spirv`. Each count uses the literal `multisampled_render_to_single_sampled` path component in the corresponding file. The non-monolithic counts include the `garbage_color_attachment` leaves, which the monolithic root does not register.
...
[`createMultisampledRenderToSingleSampledTests`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L7059-L7065) supplies this test family. [`createMultisampledRenderToSingleSampledTestsInGroup`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L7027-L7040) omits the non-dynamic path for shader-object construction types, because shader objects require dynamic rendering. The separate [`createMultisampledMiscTests`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L7067-L7071) registers `pipeline.monolithic.multisample.misc`; it is a separate test family and does not appear in this canonical tree. Its `dynamic_rendering` subtree additionally contains `emulate_msrtss`, which is described with the dynamic-rendering behavior below.
...
This path repeats supported scenarios with dynamic-rendering attachment state. Non-monolithic construction types add `garbage_color_attachment`, which probes dynamic attachment handling when a color attachment contains deliberately unusable data. Each `garbage_color_attachment` format combination is registered with both 2 and 4 samples.

The `misc` family additionally registers `emulate_msrtss` inside its own `dynamic_rendering` node for non-monolithic construction types. These cases emulate the extension's behavior with `VK_KHR_dynamic_rendering_local_read` in two passes: the first pass reads the single-sampled resolve attachments as input attachments and populates the multisampled attachments (the "unresolve" draw), and the second pass draws and blends into those values before resolving back to the single-sampled attachments. Per Vulkan issue 4687, this cannot be done in one pass because input attachments cannot be mapped to resolve attachments. The resolve attachments are cleared before the render pass, because dynamic rendering has no load-op clear for resolve attachments. Stencil-format combinations add a `_with_stencil_export` variant that unresolves stencil through `VK_EXT_shader_stencil_export`, and each color-format and sample-count combination adds a `_no_ds_read` variant that omits depth/stencil input reads.
...
|---------------------|---------------------------------------|----------|
| Sample count | Changes `perPass.numSamples`, the number of `gl_SampleID` branches, sample-specific integer vectors, and the generated expected-value alternatives; `numSamples` is 2, 4, 8, or 16. | [`initBasicPrograms`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L3225-L3247) |
| Integer color format | Selects `ivec4`/`itexture2D` for `VK_FORMAT_R16G16B16A16_SINT` or unsigned counterparts for the unsigned format. | [`initBasicPrograms`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L3182-L3183), [`initBasicPrograms`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L3309-L3310) |
| Depth/stencil format | Conditionally emits depth and/or stencil descriptors and verification code; depth expected value depends on `VK_RESOLVE_MODE_SAMPLE_ZERO_BIT` versus other modes. | [`initBasicPrograms`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L3311-L3314), [`initBasicPrograms`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L3393-L3406) |
| Render area | Push-constant origin/extent changes `uv`, dispatch bounds, and tolerance terms; the shader returns for invocations outside the configured area. | [`initBasicPrograms`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L3218-L3220) |
| MSRTSS versus ordinary multisampling | Does not change GLSL text; host generation changes image sample counts and resolve attachment setup while retaining the graphics sample count. | [`generateBasicTest`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L3604-L3657) |
...
5. The host binds the result views to the compute checker through [`setupVerifyDescriptorSetAndPipeline`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L2508-L2581). The compute dispatch compares attachment values over the target area. Partial-area helpers separately check that untouched pixels retain their expected values.
6. [`postVerifyBarrier`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L2583-L2601) makes compute writes available for host reads. The host submits, waits, reads `verificationBuffer`, and logs the diagnostic `verify` image on failure.
...
| Test parameter model | [`TestParams`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L175-L310) |
| MSRTSS image creation | [`makeImage`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L459-L485) |
| Generated basic shaders | [`initBasicPrograms`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L3161-L3426) |
| Group generation | [`createMultisampledTestsInGroup`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L6474-L7025) |
| Extension-family registration | [`createMultisampledRenderToSingleSampledTests`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L7059-L7065) |
| Control-family registration | [`createMultisampledMiscTests`](../../../modules/vulkan/pipeline/vktPipelineMultisampledRenderToSingleSampledTests.cpp#L7067-L7071) |
