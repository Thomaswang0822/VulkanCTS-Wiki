...
**Core question:** Does `OpExtInst Ldexp` from `GLSL.std.450` correctly compute `x * 2^e` for every supported combination of floating-point significand type and integer exponent type, including large negative exponents that underflow to a signed zero?
...
- All 36 test case leaves are pure Amber-dispatched cases. The C++ file only registers each case name and its feature requirements through `cts_amber::createAmberTestCase`; the SPIR-V assembly, host buffers, descriptor bindings, dispatch, and `EXPECT` checks live in the matching `.amber` files under `external/vulkancts/data/vulkan/amber/ldexp/`.
...
| Push constant `count` | `32` | Number of active Ldexp operations per dispatch. The compute shader has 64 invocations; only invocations with `gl_LocalInvocationIndex < count` execute the operation. | [ldexp_float32_int32.amber](../../../data/vulkan/amber/ldexp/ldexp_float32_int32.amber#L250-L252) |
...
All 36 Amber scripts share one compute-shader template; they differ only in declared float and integer types, the resulting `OpTypeVector` shapes, the `OpCapability` set, and the runtime-array `ArrayStride` values. The representative walkthrough uses the baseline case `ldexp_float32_int32`, which needs no optional feature and exposes the shared control flow and pass/fail logic most cleanly.
...
| Exponent buffer values | Repeated pattern `1, 0, -1, -2, -14, -2147483643, -2147483644, -2147483645` to exercise normal exponents and very large negative exponents that underflow to a signed zero. |
| `EXPECT ... TOLERANCE .0001` | Per-result tolerance check; underflowed results must equal `-0.0` or `0.0` exactly within tolerance. |

#### Purpose

This shader applies `OpExtInst Ldexp` to one `(significand, exponent)` pair per active invocation, writes the result to a storage buffer, and lets the Amber `EXPECT` block compare 32 result entries against precomputed reference values. The pass condition is that every computed result matches its reference within `0.0001`.
...
- **Pass/fail logic.** Each active invocation reads `significands[idx]` and `exponents[idx]`, computes `Ldexp`, and stores to `results[idx]`. Inactive invocations write nothing. The Amber `EXPECT results IDX <offset> ...` checks then read back the `results` buffer at byte offsets `0, 4, 8, ..., 124` (one `float` element every 4 bytes) and compare each against a precomputed reference within `0.0001`. The expected values cover both normal results (e.g., `-1.0 * 2^1 = -2.0`, `-1.0 * 2^-14 ≈ -6.103515625e-05`) and underflowed results that must equal signed zero (`-1.0 * 2^-2147483643 = -0.0`).

#### Parameter Variation Summary

The other 35 cases share the same `main` body, the same four-buffer layout (significands, exponents, results, push constant), and the same `EXPECT` strategy. They differ only in:
...
- The `OpCapability` and storage-feature requirements reported to CTS by the dispatcher so unsupported cases are skipped rather than run on devices lacking the feature.
...
- The dispatch is `RUN pipeline 1 1 1`, producing one workgroup of 64 invocations. With `pc.count = 32`, the first 32 invocations execute `Ldexp`; the remaining 32 skip the operation and write nothing.
- After the dispatch, the Amber `EXPECT` block reads 32 result entries at byte offsets `0, 4, 8, ..., 124` for the scalar case and proportionally scaled strides for the vector cases, and compares each against a precomputed reference value within `TOLERANCE .0001`.
...
| All cases share the underflow check | Large negative exponents (`-2147483643` and similar) must underflow to a signed zero; producing NaN, denormal, or wrong sign is a failure. |
...
#### Wrong underflow behavior for large negative exponents

**Possible failure symptoms:** An `EXPECT` clause for an entry whose exponent is `-2147483643`, `-2147483644`, or `-2147483645` fails. The expected value is signed zero (`-0.0` for negative significands, `0.0` for positive significands); the actual value is a denormal, a small nonzero float, NaN, or a zero with the wrong sign.

**Possible implementation causes:** SPIR-V `Ldexp` must flush to zero when `2^e` underflows the float type's minimum normal. A driver that returns a denormal, a subnormal residue, NaN, or drops the sign of the significand during the flush violates the GLSL.std.450 contract. Wide-exponent cases (`int64`) are sensitive because the magnitude of `e` is far outside the float type's normal exponent range; if the driver clamps or truncates `e` to 32 bits before the underflow check, the result can still be correct, but if it sign-extends incorrectly or treats `e` as unsigned, the underflow may not happen and a wrong value is produced.
...
- Every case shares one compute-shader template that loads one significand and one exponent per active invocation, applies `OpExtInst Ldexp`, and stores the result; cases differ only in declared float and integer types, capabilities, and runtime-array strides.
...
- The `EXPECT` block checks 32 result entries per dispatch with `0.0001` tolerance; large negative exponents (`-2147483643`, `-2147483644`, `-2147483645`) are deliberately included to verify that underflow produces a correctly signed zero.
- See `## Failure Meaning` for the failure interpretation: a failing result points to wrong `Ldexp` lowering for the active float or integer width, wrong underflow behavior, wrong vector component or stride handling, or a feature advertisement that does not match the storage access path.
