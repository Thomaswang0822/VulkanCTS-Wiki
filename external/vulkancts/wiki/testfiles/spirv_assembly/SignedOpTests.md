...
- Each leaf maps to one `.amber` script under [`spirv_assembly/instruction/compute/signed_op/`](../../../data/vulkan/amber/spirv_assembly/instruction/compute/signed_op/) that embeds SPIR-V assembly and an `EXPECT ... EQ_BUFFER` check.
...
| Dispatch width | 2, 3, 5, or 15 invocations | The number of input elements exercised by the leaf's hand-picked input buffer; matches the element count of the leaf's Amber `BUFFER ... DATA` block. | [Amber data directory](../../../data/vulkan/amber/spirv_assembly/instruction/compute/signed_op/) |
...
The storage buffer element type is signed `int`, but the atomic opcode (`OpAtomicUMax` or `OpAtomicUMin`) performs the comparison as unsigned. [`int_atomicumax`](../../../data/vulkan/amber/spirv_assembly/instruction/compute/signed_op/int_atomicumax.amber) initializes the output slot to `7` for the first element and `-1` for the eighth, then atomically combines each input with the output using unsigned comparison. Because `LocalSize 1 1 1` gives one element per workgroup, the atomicity itself is not stressed; the leaf is really checking that the unsigned comparison path is taken on a signed-typed pointer.
...
The 21 leaves share a near-identical compute shader skeleton: one `GLCompute` entry point, `LocalSize 1 1 1`, an `Input` `GlobalInvocationId` builtin, and one or two `BufferBlock` storage structs wrapping a `RuntimeArray` of the declared element type. The only leaf-to-leaf variation is the operation line and the number of input bindings. One representative walkthrough is sufficient; the behavioral group section above covers the cross-leaf variation.
...
All leaves share a common Amber infrastructure cause: a pipeline, descriptor-binding, or `EQ_BUFFER` comparison defect would fail every leaf uniformly regardless of the operation under test.
...
| Amber data directory | [`spirv_assembly/instruction/compute/signed_op/`](../../../data/vulkan/amber/spirv_assembly/instruction/compute/signed_op/) | The 21 `.amber` scripts embedding SPIR-V assembly and `EQ_BUFFER` checks. |
