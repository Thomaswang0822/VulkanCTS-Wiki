...
| `small` | Both signed 8-bit input buffers are randomized in `[-20, 20]`. |
| `ss` | Both operands use signed component types. |
| `v4i8` | Each invocation loads four signed 8-bit components from each input buffer. |
| `out32` | The dot product produces one signed 32-bit result. |
| unpacked | The instruction consumes two four-component vectors rather than two packed 32-bit scalars. |

#### Purpose

This case checks an ordinary signed four-component 8-bit dot product and writes the signed 32-bit result without accumulation or saturation. It exercises the dedicated unpacked 4×8-bit input capability path while keeping the arithmetic range small.

#### Structural Design

| Phase | Representative specialization | Role |
|-------|-------------------------------|------|
| Declare | `Int8`, `DotProductInput4x8BitKHR`, `DotProductKHR`, and `SPV_KHR_integer_dot_product` | Enables signed 8-bit vector storage and the integer dot-product instruction. |
| Address | Read `GlobalInvocationId.x`; index bindings `0` and `1` | Selects one LHS/RHS vector pair for the invocation. |
| Compute | Load two `v4i8` vectors; execute `OpSDotKHR` with a signed 32-bit result type | Multiplies corresponding signed components and sums the four products. |
| Store | Index binding `2`; store the scalar result | Produces one output element for host comparison. |
...
- Bindings `0` and `1` are `BufferBlock` input arrays with `ArrayStride 4`; binding `2` is the signed 32-bit output array with `ArrayStride 4`.
- The disassembler prints the promoted canonical spellings `DotProductInput4x8Bit`, `DotProduct`, and `OpSDot`; these are the binary encodings assembled from the CTS-authored `KHR` spellings, and the module retains `OpExtension "SPV_KHR_integer_dot_product"`.
- The host dispatches 200 workgroups of `1 × 1 × 1`, so `GlobalInvocationId.x` selects one of 200 independent vector pairs.
...
%_runtimearr_v4char = OpTypeRuntimeArray %v4char
  %_struct_3 = OpTypeStruct %_runtimearr_v4char
...
%gl_GlobalInvocationID = OpVariable %_ptr_Input_v3uint Input
      %int_0 = OpConstant %int 0
       %main = OpFunction %void None %13
         ...
         %33 = OpAccessChain %_ptr_Uniform_v4char %5 %int_0 %32
         %34 = OpLoad %v4char %33
         %35 = OpAccessChain %_ptr_Uniform_v4char %6 %int_0 %32
         %36 = OpLoad %v4char %35
         %37 = OpSDot %int %34 %36
         %38 = OpAccessChain %_ptr_Uniform_int %7 %int_0 %32
               OpStore %38 %37
               OpReturn
               OpFunctionEnd
```
...
|-------------|------|----------------|
| `compareDotProductAccSat` | [AccSat result comparison](../../../modules/vulkan/spirv_assembly/vktSpvAsmIntegerDotProductTests.cpp#L94-L183) | Conditional saturation-aware host verifier used by all AccSat families; it skips slots whose sign-separated partial products are outside the output range. |
| `addDotProductExtensionAndFeatures` | [dot-product feature setup](../../../modules/vulkan/spirv_assembly/vktSpvAsmIntegerDotProductTests.cpp#L198-L220) | Requests the core feature and width-dependent storage prerequisites. |
| `dotProductPacking` / `dotProductVector*` | [packing and vector-shape definitions](../../../modules/vulkan/spirv_assembly/vktSpvAsmIntegerDotProductTests.cpp#L222-L244) | Defines packed/signedness combinations and 8/16/32-bit vector shapes. |
| `generateIntegerDotProductCode` | [SPIR-V dot-product generation](../../../modules/vulkan/spirv_assembly/vktSpvAsmIntegerDotProductTests.cpp#L354-L528) | Generates the SPIR-V assembly and selects each instruction form. |
| `fillDotProductOutputs` / `getDotProductTestName` | [reference outputs and test names](../../../modules/vulkan/spirv_assembly/vktSpvAsmIntegerDotProductTests.cpp#L493-L539) | Builds ordinary references and exact registered leaf names. |
| `createOpSDotKHRComputeGroup` | [signed dot-product registration](../../../modules/vulkan/spirv_assembly/vktSpvAsmIntegerDotProductTests.cpp#L1175-L1190) | Registers signed non-accumulating ranges. |
| `createOpUDotKHRComputeGroup` / `createOpSUDotKHRComputeGroup` | [unsigned and mixed-sign registration](../../../modules/vulkan/spirv_assembly/vktSpvAsmIntegerDotProductTests.cpp#L1192-L1227) | Registers unsigned and mixed-signedness non-accumulating ranges. |
| `createOpSDotAccSatKHRComputeGroup` | [signed AccSat registration](../../../modules/vulkan/spirv_assembly/vktSpvAsmIntegerDotProductTests.cpp#L1229-L1256) | Registers signed saturation ranges. |
| `createOpUDotAccSatKHRComputeGroup` / `createOpSUDotAccSatKHRComputeGroup` | [unsigned and mixed-sign AccSat registration](../../../modules/vulkan/spirv_assembly/vktSpvAsmIntegerDotProductTests.cpp#L1258-L1320) | Registers unsigned and mixed-signedness saturation ranges. |
| Vulkan feature contract | [Vulkan feature contract](../../../../vulkan-docs/src/chapters/features.adoc#L6695-L6728) | Defines `shaderIntegerDotProduct` and the permitted dot-product capabilities. |
| Mustpass leaf population | [SPIR-V assembly mustpass leaves](../../../mustpass/main/vk-default/spirv-assembly.txt#L8152-L9624) | Contains the 1,392 executable leaves for these six families. |
