...
The page covers the implementation-bearing `maint9_vectorization` test family. Its direct operation-family children are shown below.
...
These cases author SPIR-V assembly directly in `M9V_Case::initPrograms()` rather than compiling GLSL or HLSL. The representative path below uses a `vec4` signed 16-bit base and result with scalar signed 32-bit `offset` and `count`. The generated instruction text is conditional on the complete operand list, so the source generator is the authoritative representation rather than a hand-copied specialization.

### Representative Shader Walkthrough 1: `bit_field_s_extract` vector case
...
The assembly is generated by [`M9V_Case::initPrograms()`](../../../modules/vulkan/spirv_assembly/vktSpvAsmMaint9VectorizationTests.cpp#L325-L717), specialized by `TestParams` and `OperandType`. The generator conditionally emits the capabilities, `ArrayStride` decorations, scalar and vector types, physical-storage-buffer pointers, and the selected `OpBit*` instruction. It uses [`SPIRV_VERSION_1_6`](../../../modules/vulkan/spirv_assembly/vktSpvAsmMaint9VectorizationTests.cpp#L713-L716) for every case.
