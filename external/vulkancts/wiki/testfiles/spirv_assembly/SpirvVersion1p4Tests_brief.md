...
```text
[host] CTS creates the spirv1p4 test group with 12 subgroup CaseGroup objects
...
The flow is uniform across all 12 subgroups; the differences are the embedded SPIR-V and the per-case feature gates.

## Generated Test Artifacts and Bound Resources

### Generated or loaded program artifacts

- Amber scripts under `external/vulkancts/data/vulkan/amber/spirv_assembly/instruction/spirv1p4/<subgroup>/<basename>.amber` are the only program artifacts. Each script embeds the SPIR-V assembly directly between `[compute shader spirv]` (or fragment/geometry/tessellation/vertex equivalents) and `[test]`. No GLSL/HLSL source is generated by CTS for this file; the SPIR-V is hand-authored test data.
...
> **Behavior parameter:** test family (the 12 registered subgroups under `spirv_assembly.instruction.spirv1p4`)
>
...
- The `entrypoint` subgroup spans 6 stages (`comp`, `frag`, `geom`, `tess_con`, `tess_eval`, `vert`) and 4 resource kinds (push constant, SSBO, UBO, workgroup). Geometry cases require `Features.geometryShader`; tessellation cases require `Features.tessellationShader`.
...
|-------|-------------|----------------|
| `createSpirvVersion1p4Group()` | [`vktSpvAsmSpirvVersion1p4Tests.cpp#L124-L409`](../../../modules/vulkan/spirv_assembly/vktSpvAsmSpirvVersion1p4Tests.cpp#L124-L409) | Top-level group creation; defines all 12 subgroups and their case lists. |
...
| Representative Amber script | [`opselect/scalar_select.amber`](../../../external/vulkancts/data/vulkan/amber/spirv_assembly/instruction/spirv1p4/opselect/scalar_select.amber) | Smallest representative walkthrough; embeds SPIR-V assembly for `OpSelect` on scalars. |
| OpCopyLogical UBO→SSBO Amber | [`opcopylogical/ubo_to_ssbo.amber`](../../../external/vulkancts/data/vulkan/amber/spirv_assembly/instruction/spirv1p4/opcopylogical/ubo_to_ssbo.amber) | Demonstrates layout conversion through `OpCopyLogical`. |
| NoSignedWrap Amber | [`wrap/no_signed_wrap.amber`](../../../external/vulkancts/data/vulkan/amber/spirv_assembly/instruction/spirv1p4/wrap/no_signed_wrap.amber) | Demonstrates integer wrap decoration. |

## Questions / Risk Points for User Audit

- Is the test-family-level primary behavioral axis correct, or should the pointer families be treated as one combined family with a secondary variable-pointer axis?
- Are the per-subgroup feature-requirement tiers documented correctly against the C++ source?
- Is the `uniformid` subgroup-size assumption (≤ 8) worth calling out in `## Case Pruning` rather than `## Important Variations`?
- Should the `#### SPIR-V` subsection omission (per the task's special instruction for Amber-backed SPIR-V pages) be documented in the page itself, or only in the completion report?

## Conversion Notes for Final Wiki Rewrite

- The brief's `### Background Knowledge` is concept-dense; the final page's `## Background Knowledge` should distill it to a short bullet list (SPIR-V 1.4 entry-point interface rule, pointer comparison/difference, `OpSelect` composite extension, `OpCopyLogical`/`OpCopyMemory` access operands, decorations folded from extensions, `UConvert` in `OpSpecConstantOp`, loop control hints).
- The brief's `## One Concrete Example` becomes the `## Shader Analysis` representative walkthrough using `opselect/scalar_select.amber`. Per the task's special instruction, the SPIR-V assembly is extracted and placed under `#### Source Code` (unfoldable), and the `#### SPIR-V` collapsed subsection is omitted to avoid duplication.
- The brief's `### Failure Cause Mapping` table is copied directly into the final page's `### Failure Cause Mapping`.
- The brief's `## What Is Checked` content moves into `## Runtime Execution and Result Checking` and `## Behavior Parameters`.
- The brief's `## Important Variations and Special Cases` content moves into `## Behavior Parameters`, `## Parameter Dimensions and Observed Values`, and `## Case Pruning` as appropriate.
- The brief's `## Source Mapping` becomes the basis for `## Source Reference Appendix`.
- The page should explicitly state that all cases are Amber-driven and that the C++ file is a registration/dispatcher (not a shader builder).
