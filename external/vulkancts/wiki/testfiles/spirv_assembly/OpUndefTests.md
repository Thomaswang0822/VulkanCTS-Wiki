...
The SPIR-V module is generated by `OpUndefBufferComputeTestCase::initPrograms` from a fixed template. The template varies the uniform/storage class, array shape, and live/dead control-flow parameters; `%undef_res` is passed to the helper function, and live-code cases execute `OpCompositeExtract` while dead-code cases bypass the call with `OpConstantFalse`.
...
; Version: 1.3
    ...
    %false = OpConstantFalse %bool
     ...
     %entry = OpLabel
               OpBranchConditional %false %then %merge
      %then = OpLabel
%undef_res = OpUndef %int
               OpBranch %merge
     %merge = OpLabel
               OpReturn
               OpFunctionEnd
