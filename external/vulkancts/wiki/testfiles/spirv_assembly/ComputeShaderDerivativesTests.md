...
| Quad workgroup size | `4_4_1`, `4_8_1`, `8_4_1`, `8_8_1` | Varies the compute quad layout for derivative-value cases; local-invocation-index cases retain `4_4_1`. | [quad-size registration](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3780-L3825) |
...
- The SPIR-V target version is **1.3 for compute and fragment shaders** and **1.4 for mesh and task shaders** ([target version selection](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3680-L3717)).
...
| `DerivativeFeature` | `linear` uses `ComputeDerivativeGroupLinearKHR`/`DerivativeGroupLinearKHR` and masks the linear index; `quads` uses `ComputeDerivativeGroupQuadsKHR`/`DerivativeGroupQuadsKHR` and derives a 2×2 quad index from x/y. | [`getDerivativeCapability`/`getDerivativeExecutionMode`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L296-L318), [`getTestValueCode`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L359-L438) |
| `DerivativeVariant` | For `derivative_value`, `normal`, `fine`, and `coarse` substitute unsuffixed, `*Fine`, or `*Coarse` derivative opcodes. Fine cases use variant-dependent expected values; normal and coarse cases use the common expected derivative pattern. | [`getDxFunc`/`getDyFunc`/`getWidthFunc`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3482-L3521), [`checkResult`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L1861-L2432) |
...
- The host picks `shaderType`, `testType`, `feature`, `variant` (for `derivative_value`), `dataType`, `quadOp`/`quadNdx` (for `quad_op`), `mipLvl` (for `lod_op`), and `numWorkgroup`, then builds the SPIR-V assembly via two `tcu::StringTemplate` specialization passes per `testType` and registers it with `spirvAsmSources.add("compute" | "mesh" | "task")` ([`ComputeShaderDerivativeCase::initPrograms()`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3678-L3717)).
...
- After `submitCommandsAndWait`, the host runs [`checkResult`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L1861-L2432) which builds expected buffers on the CPU from the same per-`testType`/`feature`/`variant`/`dataType` rules and compares against the readback buffers. For `LOD_QUERY` the comparison uses `compareFloats(a, b, 0.015 + (lodMax-lodMin)/2)` for the computed LOD slot; for every other `testType` the comparison is exact.
...
|-------------|------|----------------|
| `createComputeShaderDerivativesTests` | [`createComputeShaderDerivativesTests`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3729-L4176) | Registration root: builds the `compute_shader_derivatives` group and all leaves |
| `ComputeShaderDerivativeCase::checkSupport` | [`ComputeShaderDerivativeCase::checkSupport`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L2830-L2907) | Feature/property/subgroup gating logic for every test case |
| `ComputeShaderDerivativeInstance::iterate` | [iterate](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L2415-L2822) | Host-side command buffer recording, dispatch/draw, and result check entry |
| `ComputeShaderDerivativeInstance::checkResult` | [`ComputeShaderDerivativeInstance::checkResult`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L1842-L2413) | Per-testType expected-value generation and comparison logic |
| `ComputeShaderDerivativeCase::initPrograms` (COMPUTE template) | [`initPrograms` COMPUTE branch](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3068-L3187) | The SPIR-V assembly string template for the compute variant |
| `ComputeShaderDerivativeCase::initPrograms` (MESH template) | [`ComputeShaderDerivativeCase::initPrograms()`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3188-L3353) | The SPIR-V assembly string template for the mesh variant |
| `ComputeShaderDerivativeCase::initPrograms` (TASK template) | [`ComputeShaderDerivativeCase::initPrograms()`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3354-L3477) | The SPIR-V assembly string template for the task variant |
| `TestType::DERIVATIVE_VALUE` specMap | [`DERIVATIVE_VALUE` specMap](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3482-L3521) | First and second specMap for derivative_value cases |
| `TestType::VERIFY_NDX` specMap | [`ComputeShaderDerivativeCase::initPrograms()`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3522-L3556) | specMap for verify_ndx cases (subgroup id and invocation id stores) |
| `TestType::QUAD_OPERATIONS` specMap | [`quad_op` registration and opcode mapping](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3557-L3588) | specMap for quad_op cases |
| `TestType::LOD_SAMPLE` specMap | [`ComputeShaderDerivativeCase::initPrograms()`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3589-L3629) | specMap for lod_op.sample cases |
| `TestType::LOD_QUERY` specMap | [`ComputeShaderDerivativeCase::initPrograms()`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3630-L3671) | specMap for lod_op.query cases |
| SPIR-V target version per shader type | [target version selection](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L3680-L3717) | 1.3 for compute/fragment, 1.4 for mesh/task |
| `getTestValueCode` | [`getTestValueCode`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L345-L424) | Builds the per-feature/variant/dataType SPIR-V fragment that computes `%test_value` |
| `genTexCoords` | [`genTexCoords`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L489-L518) | Builds the per-feature/mipLvl SPIR-V fragment that computes texture coordinates (lod_op) |
| `getDerivativeCapability` / `getDerivativeExecutionMode` | [`getDerivativeCapability` / `getDerivativeExecutionMode`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L282-L303) | Maps `DerivativeFeature` to capability/execution-mode strings |
| `getLinearNdxMul` | [`getLinearNdxMul(DERIVATIVE_VALUE)`](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L426-L462) | Per-testType multiplier for the linear `ndx` computation |
| Swap-value generators | [Swap-value generators](../../../modules/vulkan/spirv_assembly/vktSpvAsmComputeShaderDerivativesTests.cpp#L621-L1123) | Host-side expected buffers for the three swap directions (quad_op swap) |
...
| Mustpass leaves | [Mustpass leaves](../../../mustpass/main/vk-default/spirv-assembly.txt) | All `compute_shader_derivatives.*` leaves in the default mustpass |
