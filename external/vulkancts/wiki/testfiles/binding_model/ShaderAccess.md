...
**Core question:** Does a descriptor bound in the selected command buffer deliver the intended resource data to every active shader stage?
...
- The direct behavior split is where the test records descriptor binding and the consuming draw: `primary_cmd_buf` or `secondary_cmd_buf`.
...
| Descriptor bind command | `bind`, `bind2` | Selects `vkCmdBindDescriptorSets` or `vkCmdBindDescriptorSets2`. | [bind-command registration](../../../modules/vulkan/binding_model/vktBindingShaderAccessTests.cpp#L9855-L9863), [binding helper](../../../modules/vulkan/binding_model/vktBindingShaderAccessTests.cpp#L478-L508) |
...
- `bind` records `vkCmdBindDescriptorSets`. `bind2` records `vkCmdBindDescriptorSets2` with the pipeline layout, set range, descriptor sets, dynamic offsets, and existing stage mask.
...
- Compute cases bind the same descriptor layouts to a compute pipeline, apply host-write to shader-read barriers, dispatch four one-invocation workgroups, and apply a shader-write to host-read barrier. The shader writes one `vec4` per workgroup to a result buffer initialized to `-1`.
- A graphics case passes only when `bilinearCompare` accepts the full reference image. A compute case passes only when all four result vectors equal the calculated references. Four untouched `-1` values produce `Result buffer was not written to`; other mismatches produce `Invalid result values`.
...
- `bind2` buffer cases require `VK_KHR_maintenance6`.
...
| Compute submission and checking | [ComputeCommand::submitAndWait()](../../../modules/vulkan/binding_model/vktBindingShaderAccessTests.cpp#L2226-L2317), [BufferComputeInstance::testResourceAccess()](../../../modules/vulkan/binding_model/vktBindingShaderAccessTests.cpp#L2804-L3008) | Records binding, barriers, dispatch, readback, and exact result checks. |
...
| Support checks | [verifyDriverSupport()](../../../modules/vulkan/binding_model/vktBindingShaderAccessTests.cpp#L164-L259), [bind2 support](../../../modules/vulkan/binding_model/vktBindingShaderAccessTests.cpp#L3631-L3635) | Defines extension, feature, image, stage, and maintenance6 requirements. |
| Mustpass representative | [binding-model.txt](../../../mustpass/main/vk-default/binding-model.txt#L65729) | Confirms the exact case reconstructed in the shader walkthrough. |
