...
- Generated layout, readonly, and physical-storage cases use `MultiQueueRunnerTestInstance` with `COMPUTE_QUEUE`. Each `queuePassImpl()` takes its submission queue and command-pool family from `QueueData` and clears buffers and allocations left by the previous pass before rebuilding resources ([queue pass](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2319-L2332)). Queue passes repeat the same registered case and result checks; they do not add mustpass leaves.
...
- [`SSBOLayoutCase::delayedInit()`](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2823-L2860) computes the reference layout, allocates initial and expected-write reference storage, fills deterministic values, preserves fields the shader will not write, and generates the compute shader.
...
- In `per_block_buffer` mode, each block gets its own storage buffer. In `single_buffer` mode, the harness packs blocks into one buffer and rounds each binding offset to `minStorageBufferOffsetAlignment` ([`queuePassImpl()`](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2400-L2463)).
- The `phys` path adds `VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT`, queries `VkDeviceAddress` values for the descriptors, and pushes those addresses before the dispatch ([`queuePassImpl()`](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2491-L2536)).
- Each queue pass dispatches one compute workgroup, records shader-write to host-read buffer barriers, waits for its selected queue, invalidates mapped allocations, and checks the pass counter and byte-for-byte data comparison ([`queuePassImpl()`](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2319-L2680)).
...
- The generated 64-bit-indexing cases require a device-local heap of at least `(1ull << 32) + (1ull << 28)` bytes. [`delayedInit()`](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2823-L2860) checks this through [`deviceHasHeapForSsbo64bAllocation()`](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2755-L2768) and skips the host-side reference allocation when the heap is too small, and [`checkSupport()`](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2771-L2821) then reports `NotSupportedError`. This is a heap-capacity precondition, not a guarantee of currently free memory.
- [`SSBOLayoutCase::checkSupport()`](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2771-L2821) skips generated cases that require unsupported relaxed block layout, 16-bit or 8-bit storage, scalar block layout, buffer device address, descriptor indexing, runtime descriptor arrays, 64-bit indexing, or more storage-buffer descriptors than the device limit permits. Outside Vulkan SC, every 64-bit-indexing case is additionally reported `NotSupportedError` on a 32-bit build, because its host reference data exceeds what a 32-bit `size_t` can represent; `delayedInit()` skips that allocation first, and the storage builder asserts instead of silently truncating the total ([32-bit gate](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2795-L2799), [skip](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2826-L2830), [assert](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2153-L2156)).
...
- The same out-of-memory downgrade applies to the generated layout cases: outside Vulkan SC, [`SSBOLayoutCaseInstance::queuePass()`](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2297-L2317) turns a `vk::OutOfMemoryError` raised while running the pass into `NotSupportedError` when the host reference data is at least 4 GiB, and rethrows it otherwise.
...
|-------------|------|----------------|
| Category dispatcher | [`createTests()`](../../../modules/vulkan/ssbo/vktSSBOLayoutTests.cpp#L2246-L2266) | Registers the five direct `ssbo` children and mode wrappers |
| Generated family registration | [`SSBOLayoutTests::init()`](../../../modules/vulkan/ssbo/vktSSBOLayoutTests.cpp#L1308-L2199) | Defines fixed and seeded layout case dimensions |
...
| Reference layout and generated shader | [`SSBOLayoutCase::delayedInit()`](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2823-L2860) | Builds reference data and compute source |
...
| Queue pass dispatch | [`SSBOLayoutCaseInstance::queuePass()`](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2297-L2317) | Wraps the pass body and downgrades large out-of-memory failures |
| Layout execution and result checks | [`queuePassImpl()`](../../../modules/vulkan/ssbo/vktSSBOLayoutCase.cpp#L2319-L2680) | Binds resources, dispatches, synchronizes, and compares results |
