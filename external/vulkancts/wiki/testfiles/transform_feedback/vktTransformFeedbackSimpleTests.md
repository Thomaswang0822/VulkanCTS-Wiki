...
The monolithic group includes the same broad generator families as the GPL variants plus source-observed cases that are emitted only for monolithic construction. For example, the shader-object rebind case is guarded by a monolithic-only condition in [`createTransformFeedbackSimpleTests()`](../../../modules/vulkan/transform_feedback/vktTransformFeedbackSimpleTests.cpp#L6977-L6984), explaining why this root has 7894 observed MUSTPASS entries while each GPL sibling has 7886.
