...
The ten names are registered by [`createAcquireDrmDisplayTests`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L757-L782). The `wsi.acquire_drm_display` group is attached directly to the WSI test category by [`createWsiTests`](../../../modules/vulkan/wsi/vktWsiTests.cpp#L85-L100). The same ten paths appear in the default Vulkan mustpass list, for example [`vk-default/wsi.txt`](../../../mustpass/main/vk-default/wsi.txt#L1-L10).
...
| DRM file descriptor | A device fd, a second fd, an invalid fd opened on `/`, or a DRM lease fd, depending on the test case | Changes whether the command receives a device-associated descriptor, a descriptor without master status, an unusable descriptor, or a descriptor that owns only leased DRM objects. | [`getDrmFdPtr`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L197-L224), [`testGetDrmDisplayEXTInvalidFd`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L340-L362), [`testAcquireDrmDisplayEXTUnownedConnectorId`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L622-L662) |
| Connector ownership | A connected connector, `connectorId + 1234`, or a second connector excluded from a DRM lease | Controls whether `vkGetDrmDisplayEXT` can identify a display and whether the leased descriptor owns the requested connector. | [`getConnectedConnectorId`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L238-L256), [`testGetDrmDisplayEXTUnownedConnectorId`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L447-L482) |
| DRM permissions | Master fd or a second fd used as the non-master fd | Separates operations that only gather display information from acquisition and release operations that require control of the display. | [`isDrmMaster`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L291-L299), [`testGetDrmDisplayEXTNotMaster`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L404-L430) |
...
- The case-level [`checkSupport`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L730-L736) requires `VK_KHR_surface`, `VK_KHR_display`, `VK_EXT_direct_mode_display`, and `VK_EXT_acquire_drm_display`, so a platform that lacks any of them is reported not supported before an instance is created. The instance then enables the same four extensions in [`createInstanceWithAcquireDrmDisplay`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L179-L189).
...
- Missing instance extension support is pruned in `checkSupport`, before any instance exists. Runtime prerequisite checks can then report `NotSupportedError` when the build lacks DRM support, no matching DRM primary node exists, no connected connector exists, no compatible CRTC exists, or the environment cannot provide the requested DRM ownership state.
...
| Required instance extensions | [`checkSupport`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L730-L736), [`createInstanceWithAcquireDrmDisplay`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L179-L189) | The former rejects unsupported platforms before execution, the latter enables the same extensions on the created instance. |
| DRM device discovery | [`getDrmFdPtr`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L197-L224) | Maps the selected physical device to its DRM primary node. |
| Connector and CRTC helpers | [`getConnectedConnectorId` and `getValidCrtcId`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L238-L256) | Select connected connectors and a CRTC for DRM leases. |
| Master detection | [`isDrmMaster`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L291-L299) | Determines whether the environment can run master-dependent cases. |
...
| Release case | [`testReleaseDisplayEXT`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L678-L712) | Checks the lookup, acquire, and release sequence. |
| Registration | [`createAcquireDrmDisplayTests`](../../../modules/vulkan/wsi/vktWsiAcquireDrmDisplayTests.cpp#L757-L782) | Provides the exact test case names. |
| WSI dispatcher | [`createWsiTests`](../../../modules/vulkan/wsi/vktWsiTests.cpp#L85-L100) | Places the family at `wsi.acquire_drm_display`. |
