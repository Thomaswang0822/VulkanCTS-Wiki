...
- The dispatcher creates the category root and attaches eight direct children: `basic`, `cache`, `properties`, `descriptor_buffer`, `update_after_bind`, `image_aliasing`, `external_memory`, and `specialization_constants`.
...
The dispatcher has no parameterized test cases. It passes the test context and supplied root name to the root constructor, then delegates parameter generation to the eight child factories. The child pages should document the matrices that those factories generate.
...
- `specialization_constants` covers scalar, boolean, composite, replicated composite, and operation specialization-constant paths. The source registers these families in `specializationConstantsTestsGroup`.
- `image_aliasing` covers tensor/image aliasing with shader-based fill and verification.
- `descriptor_buffer`, `update_after_bind`, and `external_memory` cover their respective data-graph resource and descriptor integration paths.
...
|----------------------------------------|---------------------------|
| `basic`, `cache`, `properties`, `descriptor_buffer`, `update_after_bind`, `image_aliasing`, `external_memory`, or `specialization_constants` | The category root was not registered, a direct child was omitted or renamed, or the child factory failed while constructing its test group |
...
The dispatcher intentionally exposes exactly eight direct children. It does not add implementation-level cases or collapse the child families into one group. Parameter and combination pruning belongs to the child factories and shared utilities, not to this root implementation.

## Key Takeaways

- `data_graph` is a dispatcher/root implementation with eight direct children: `basic`, `cache`, `properties`, `descriptor_buffer`, `update_after_bind`, `image_aliasing`, `external_memory`, and `specialization_constants`.
