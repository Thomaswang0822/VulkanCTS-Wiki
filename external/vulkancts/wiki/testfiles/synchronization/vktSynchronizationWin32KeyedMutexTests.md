...
Registered in the LEGACY synchronization path via `createWin32KeyedMutexTest()` added to the `synchronization` group in [vktSynchronizationTests.cpp](../../../modules/vulkan/synchronization/vktSynchronizationTests.cpp) (line 141).

## Test Hierarchy

```
win32_keyed_mutex
+-- <writeOp>_<readOp>            (per operation pair)
    +-- <resource>_<handleSuffix> (per resource and handle type)
```

### Group Structure Detail

- **Operation pair groups**: Named `<writeOp>_<readOp>` (e.g., `copy_copy`, `copy_read`). Iterates over `s_writeOps` x `s_readOps` from `vktSynchronizationOperationTestData`.
...
| Family | Description |
|--------|-------------|
| Win32KeyedMutexTestCase | Individual test case per resource/handle-type/operation combination. Creates DX11 resources with keyed mutex, imports into Vulkan, writes via Vulkan, copies via DX11, reads back via Vulkan, and verifies data integrity. |
