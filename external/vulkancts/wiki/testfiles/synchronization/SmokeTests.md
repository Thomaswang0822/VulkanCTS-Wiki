...
- The legacy-only `queue_type_image_barrier_external` case creates a 16 × 16 image, transitions it from `UNDEFINED` to `GENERAL`, then records a release from the executing queue family to `VK_QUEUE_FAMILY_EXTERNAL` while changing the layout to `COLOR_ATTACHMENT_OPTIMAL`. It submits and waits without drawing or comparing pixels; successful completion is the verdict ([`externalQueueFamilyImageBarrier()`](../../../modules/vulkan/synchronization/vktSynchronizationSmokeTests.cpp#L1659-L1703)). Unlike the `queue_type_ignore_*` cases, this barrier uses different source and destination queue-family indices.
