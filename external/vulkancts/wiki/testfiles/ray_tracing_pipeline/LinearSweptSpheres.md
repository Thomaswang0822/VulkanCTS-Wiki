...
This page covers one test family registered from [`main()`](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L960-L1149):
...
|-----------|-------------------|----------------------|----------|
| Geometry type | `spheres`, `lss` | Selects the BLAS geometry type: standalone spheres or linear swept spheres. Changes the hit-classification built-in checked by the shader and the expected hit-count formula. | [Geometry type](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L1030-L1033) |
| Test type | `vertices`, `indices`, `indexing_mode_list`, `indexing_mode_successive` | Controls how the BLAS uses vertex and index data. `vertices` uses all vertices without an index buffer. `indices` uses an index buffer with `VK_INDEX_TYPE_UINT32`. `indexing_mode_list` and `indexing_mode_successive` select the LSS indexing mode. | [Test type](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L1035-L1038) |
| BLAS copy | `no_blascopy`, `blascopy` | When enabled, the test compact-copies the built BLAS before traversal, exercising the copy path for the new geometry types. | [BLAS copy](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L994-L998) |
| Endcaps | `no_endcaps`, `endcaps` | Controls whether LSS segment endpoint spheres are part of the geometry. Changes the expected hit count. Pruned for `spheres` because standalone spheres are always full spheres. | [Endcaps](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L1000-L1007) |
| Ray query | `no_use_ray_query`, `use_ray_query` | When enabled, the raygen shader uses `rayQueryEXT` instead of `traceRayEXT`. Hit classification uses `rayQueryIsSphereHitNV` or `rayQueryIsLSSHitNV` inside the raygen shader. | [Ray query](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L1009-L1016) |
| Hit object | `no_use_hit_object`, `use_hit_object` | When enabled, the raygen shader uses `hitObjectTraceRayNV` from `GL_NV_shader_invocation_reorder`. Hit classification uses `hitObjectIsSphereHitNV` or `hitObjectIsLSSHitNV`. | [Hit object](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L1018-L1025) |
| Vertex format | `float3`, `float2`, `half3`, `half2` | Controls vertex position format: 3D or 2D (z implied zero), 32-bit or 16-bit float. Maps to `VK_FORMAT_R32G32B32_SFLOAT`, `VK_FORMAT_R32G32_SFLOAT`, `VK_FORMAT_R16G16B16_SFLOAT`, `VK_FORMAT_R16G16_SFLOAT`. | [Vertex format](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L1040-L1045) |
| Radius format | `float`, `half` | Controls radius data format: `VK_FORMAT_R32_SFLOAT` or `VK_FORMAT_R16_SFLOAT`. | [Radius format](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L1047-L1050) |
...
|---------------------|---------------------------------------|----------|
| Geometry type (`spheres`) | Replaces the 12-vertex LSS array with a 12-vertex sphere array in a different order. The closest-hit shader switches to `gl_HitIsSphereNV && !gl_HitIsLSSNV`. | [Geometry type (spheres)](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L798-L815) |
| Endcaps (`no_endcaps`) | Replaces the 12-vertex array with a 5-vertex `noendCapsVertices` array and loops 5 times instead of 12. | [Endcaps (noendcaps)](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L834-L847) |
| Ray query (`use_ray_query`) | Replaces `traceRayEXT` with `rayQueryInitializeEXT` + `rayQueryProceedEXT` + `rayQueryGetIntersectionTypeEXT`. Sets `hitValue = int(rayQueryIsLSSHitNV(rq, true))` inside the raygen shader. Closest-hit shader is not invoked. | [Ray query (userayquery)](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L856-L883) |
| Hit object (`use_hit_object`) | Replaces `traceRayEXT` with `hitObjectTraceRayNV` + `reorderThreadNV` + `hitObjectIsHitNV`. Sets `hitValue = int(hitObjectIsLSSHitNV(hObj))` inside the raygen shader. | [Hit object (usehitobject)](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L885-L908) |
...
|-------------|------|----------------|
| `TestParams` struct | [TestParams struct](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L174-L185) | Per-case parameters: geometry type, test type, BLAS copy, endcaps, ray query, hit object, vertex format, radius format. |
| `DeviceHelper` custom device creation | [DeviceHelper custom device creation](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L101-L172) | Creates a custom device with required extensions and features, without `VK_KHR_pipeline_library`. |
| `iterate` validation | [iterate validation](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L249-L454) | Host-side result checking: expected hit counts for each geometry type, test type, and endcap combination. |
| `SpheresTestInstance::setupRayTracingPipeline` | [SpheresTestInstance::setupRayTracingPipeline](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L474-L504) | Builds the ray tracing pipeline with `ALLOW_SPHERES_AND_LINEAR_SWEPT_SPHERES_BIT_NV` and optional `SKIP_BUILT_IN_PRIMITIVES_BIT_KHR`. |
| `SpheresTestInstance::setupAccelerationStructures` | [SpheresTestInstance::setupAccelerationStructures](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L506-L568) | Builds sphere BLAS with 16 vertices, radii, and optional index buffer. |
| `LSSpheresTestInstance::setupAccelerationStructures` | [LSSpheresTestInstance::setupAccelerationStructures](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L614-L717) | Builds LSS BLAS with vertex, radius, index data, indexing mode, and endcap configuration. |
| `checkSupport` | [checkSupport](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L747-L768) | Feature checks for acceleration structure, ray tracing pipeline, and LSS extension. |
| `initPrograms` raygen generation | [initPrograms raygen generation](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L770-L921) | Generates the raygen shader with branches for geometry type, endcaps, ray query, and hit object. |
| `initPrograms` closest-hit and miss | [initPrograms closest-hit and miss](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L923-L955) | Generates the closest-hit and miss shaders. |
| `createLinearSweptSpheresTests` registration | [register valid linear-swept-sphere parameter combinations](../../../modules/vulkan/ray_tracing/vktRayTracingLinearSweptSpheresTests.cpp#L960-L1149) | Registers the test hierarchy with pruning rules for invalid combinations. |
| `addSphereGeometry` helper | [addSphereGeometry helper](../../../framework/vulkan/vkRayTracingUtil.cpp#L905-L939) | Framework helper that creates sphere or LSS geometry with the specified parameters. |
