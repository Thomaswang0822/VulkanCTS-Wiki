...
The `indexing_*` matrix is built by nested loops over shader test type, SBT buffer offset, shader-record presence, SBT record offset, and SBT record stride [`ShaderTestTypeData()`](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L1622-L1722). The `handle_alignment` matrix is a single loop over power-of-two alignments with a paired long-vector variant on non-VulkanSC builds [`ShaderRecordData()`](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L1724-L1743).
...
- The `handle_alignment` chit shader reads `srb.data[i]` from `layout(shaderRecordEXT, std430) buffer srbBlock` and writes it to `ssbo.data[gl_LaunchIDEXT.x][i]`. The element type, element count, and shader-record extension are selected at GLSL generation time based on the alignment value [`ShaderGroupHandleAlignmentInstance()`](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L1324-L1398).
...
- **SBT construction.** The active region (hit for HIT, miss for MISS, callable for CALL) is given a non-zero `shaderBindingTableOffset = sbtOffset * shaderGroupBaseAlignment` from the start of its buffer. When `shaderRecordPresent`, the active region uses stride `deAlign32(shaderGroupHandleSize + sizeof(tcu::UVec4), shaderGroupHandleSize)` and the host writes `uvec4(idx, 0, 0, 0)` after each shader-group handle, then flushes the writes [lay out shader-binding tables and initialize record data](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L456-L639).
...
- **Reference image and pass/fail.** `verifyImage` recreates the same checkerboard corners and instance ordering, then sets each hit pixel's reference value to the family-specific formula and each miss pixel's value to the family-specific miss value. The result is compared with `tcu::intThresholdCompare` at zero tolerance [derive and compare per-pixel shader indices](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L641-L709).
- **`handle_alignment` runtime.** The host builds a single BLAS with `geoCount = 32/alignment + 1` triangle geometries translated along X, and a TLAS with one instance. It creates the hit SBT with stride `shaderGroupHandleSize + alignment`, passing per-geometry shader-record data and disabling autoalign so the explicit alignment is preserved. After `cmdTraceRays` over `geoCount x 1 x 1`, the host reads back the SSBO and compares it byte-by-byte against the per-geometry shader-record data, logging the first mismatch [trace with aligned shader records and validate output bytes](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L1405-L1613).
...
- All cases require `VK_KHR_acceleration_structure` and `VK_KHR_ray_tracing_pipeline`, with `rayTracingPipeline == VK_TRUE` and `accelerationStructure == VK_TRUE` [`ShaderBindingTableIndexingTestInstance()`](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L765-L780), [handle_alignment support](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L1278-L1289).
...
|-------------|------|----------------|
| Category root registration | [Category root registration](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L1617-L1646) | Builds the `shader_binding_table` group and attaches all four direct children. |
| Indexing family loop | [enumerate SBT hit, miss, and callable indexing variants](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L1622-L1722) | Defines the `indexing_hit`/`indexing_miss`/`indexing_call` matrix and the extra-bits scheme. |
| Handle-alignment family loop | [combine handle alignments with long-vector variants](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L1724-L1743) | Defines the alignment values and long-vector variants. |
| `TestParams` struct | [TestParams struct](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L110-L122) | Captures sbtOffset, sbtRecordOffset/Stride values, the versions passed to traceRay, and shaderRecordPresent. |
| UBO fill per family | [fill ray parameters with SBT offsets, strides, and miss indices](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L268-L304) | Maps `trParams.x/y/z` to sbtRecordOffset, sbtRecordStride, or missIndex per family. |
| SBT construction per family | [lay out aligned shader records and their buffer contents](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L456-L639) | Shows `shaderBindingTableOffset` math, shader-record aligned size, and shader-record buffer fills. |
| Reference image computation | [compute the expected shader index for each pixel](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L641-L709) | Host-side re-derivation of expected per-pixel shader index for each family. |
| Indexing support checks | [check features for SBT indexing tests](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L765-L780) | Feature gates for the indexing families. |
| Indexing shader generation | [Indexing shader generation](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L782-L944) | Generates rgen, chit_*, miss_*, call_*, and the *_shaderRecord variants. |
| Indexing runtime | [Indexing runtime](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L961-L1124) | Builds resources, runs `cmdTraceRays`, copies back, and verifies the result image. |
| Handle-alignment params struct | [Handle-alignment params struct](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L1169-L1237) | Defines alignment, geometry count, element type, and per-geometry record data. |
| Handle-alignment support checks | [Handle-alignment support checks](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L1278-L1322) | Feature and property gates for the alignment family. |
| Handle-alignment shader generation | [generate hit shaders for handle alignment and data types](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L1324-L1398) | Parameterizes the chit shader by alignment, element type, and long-vector usage. |
| Handle-alignment runtime | [trace with explicitly aligned SBT records and verify bytes](../../../modules/vulkan/ray_tracing/vktRayTracingShaderBindingTableTests.cpp#L1405-L1613) | Builds AS, pipeline, SBT with explicit alignment, runs `cmdTraceRays`, and verifies SSBO contents byte-by-byte. |
