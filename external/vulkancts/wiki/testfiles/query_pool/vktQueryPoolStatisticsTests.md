...
### compute_shader_invocations — Compute shader invocation statistics

Registered first in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L6406). It covers:
...
### input_assembly_vertices — Input-assembly vertex counter

Uses `VK_QUERY_PIPELINE_STATISTIC_INPUT_ASSEMBLY_VERTICES_BIT` and is built in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L6495). Coverage includes:
...
### input_assembly_primitives — Input-assembly primitive counter

Uses `VK_QUERY_PIPELINE_STATISTIC_INPUT_ASSEMBLY_PRIMITIVES_BIT` and is registered in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L6687). It expands the previous family across primitive topologies:
...
### vertex_shader_invocations — Vertex shader invocation counter

Uses `VK_QUERY_PIPELINE_STATISTIC_VERTEX_SHADER_INVOCATIONS_BIT` and is registered in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L7203). Its structure closely parallels `input_assembly_primitives`, including:
...
### fragment_shader_invocations — Fragment shader invocation counter

Uses `VK_QUERY_PIPELINE_STATISTIC_FRAGMENT_SHADER_INVOCATIONS_BIT` and is generated in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L7480). It differs slightly from earlier groups:
...
### geometry_shader_invocations — Geometry shader invocation counter

Uses `VK_QUERY_PIPELINE_STATISTIC_GEOMETRY_SHADER_INVOCATIONS_BIT` and is built in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L7698). Coverage includes:
...
### geometry_shader_primitives — Geometry shader primitive counter

Uses `VK_QUERY_PIPELINE_STATISTIC_GEOMETRY_SHADER_PRIMITIVES_BIT` and is registered in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L7915). It mirrors the previous family but validates primitive counts emitted by the geometry stage.
...
### clipping_primitives — Clipping primitive counter

Uses `VK_QUERY_PIPELINE_STATISTIC_CLIPPING_PRIMITIVES_BIT` and follows the same helper-driven structure in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L8335). It likewise expands into stage-specific leaf tests under each logical stem.
...
### tes_control_patches — Tessellation control shader patch counter

Uses `VK_QUERY_PIPELINE_STATISTIC_TESSELLATION_CONTROL_SHADER_PATCHES_BIT` and is registered in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L8542). It covers:
...
### vertex_only — Vertex-only rendering subset

Populated in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L9049). It mirrors only three statistics families:
...
### host_query_reset — Host-side query reset variants

Collects host-reset replicas of most major families; assembly starts in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L9054). Uses `RESET_TYPE_HOST` reset mode.

### reset_before_copy — Reset-before-copy variants

Collects cases that copy statistics results after a reset-before-copy flow; group assembly starts in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L9067). Uses `RESET_TYPE_BEFORE_COPY` reset mode.

### reset_after_copy — Reset-after-copy variants

Collects cases where queries are reset after results have been copied; group assembly starts in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L9080). Uses `RESET_TYPE_AFTER_COPY` reset mode.

### multiple_queries — Combined statistics query validation

Populated in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L8942). It validates combined statistics-query behavior when multiple query statistic bits are enabled at once.
...
### multiple_geom_stats — Multiple geometry statistics

Populated in [`QueryPoolStatisticsTests::init()`](../../../modules/vulkan/query_pool/vktQueryPoolStatisticsTests.cpp#L9019). It registers exactly eight cases from the Cartesian product of:
