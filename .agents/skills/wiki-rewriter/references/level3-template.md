...
Formatting contract for the whole Level-3 page:
...
The parseable hierarchy has a strict mechanical shape:

- Put the complete tree set in exactly one fenced `text` block. A page may contain multiple independent trees inside that one fence.
- Separate independent trees by exactly one or more blank lines. Do not put prose between trees inside the fence.
- Begin every tree with a concrete category-qualified root such as `memory_model.shared`.
- Do not include the global `dEQP-VK.` package prefix in a root.
- Expand each root by at most one direct-child level. Child lines use only `├── <component>` or `└── <component>`.
- A child may have one trailing parenthesized reader note, for example `(registration only)`. Do not use `#` comments.
- Do not use nested tree markers, placeholders, `<...>`, `...`, shorthand ranges, trailing `/`, or descriptive prose as tree nodes.
- Do not repeat roots or direct children. Multiple roots in one page must not overlap as ancestor and descendant.
- Every root and reconstructed `root.direct_child` prefix must be backed by current registration/mustpass evidence.
...
Put deeper test case leaves and large generated ranges in `## Behavior Parameters`,
...
- Do not hand-edit the walkthrough heading/subsection shape. `shader-analyzer/references/output-template.md` owns the exact
  `Representative path:`, table, source-fence, `Additional Info`, and multi-stage H5 contract; `shader-disassembler` owns each
  generated SPIR-V artifact.
...
- After insertion or regeneration, run `verify_english_structure.py` on the page. Its walkthrough, fixed-label, table, SPIR-V, and
  H5 checks are the completion authority even when the prose is semantically correct.
